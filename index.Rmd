...
A spreadsheet is also available: <a href="https://github.com/mcc-apsis/AR6-Emissions-trends-and-drivers/tree/master/Results/Public%20data/ipcc_ar6_data_growth_rates.xlsx">link</a>
...
rates %>% 
  ...
  select(-category,`Region category`=subcategory,Region=var,`1990`:`1990-2019`) %>% 
  kbl() %>% 
  add_header_above(c("","","Emissions (GtCO2eq)"=4,"Growth rate (%/yr)"=4)) %>% 
  kable_styling(bootstrap_options = c("striped")) %>%
...
rates %>% 
  ...
  select(-category,Region=subcategory,Country=var,`1990`:`1990-2019`) %>% 
  kbl() %>% 
  add_header_above(c("","","Emissions (MtCO2eq)"=4,"Growth rate (%/yr)"=4)) %>% 
  kable_styling(bootstrap_options = c("striped")) %>%
  scroll_box(width = "100%", height = "400px")

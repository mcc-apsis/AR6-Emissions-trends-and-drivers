...
ch11 <- edgar_GHG %>% 
  filter(IPCC_AR5_chapter==11) %>%
  group_by(Year,sector_code,category_final) %>% 
  summarise(CO2=sum(CO2,na.rm=T),CH4=sum(CH4,na.rm=T),N2O=sum(N2O,na.rm=T),Fgas=sum(Fgas,na.rm=T),GHG=sum(GHG,na.rm=T)) %>% 
  filter(Year==2017)

#openxlsx::write.xlsx(ch11,file="Results/edgar_AFOLU.xlsx")

```

### Total GHGs, split by region

```{r ghgs_total_income, echo=FALSE,warning=FALSE,fig.width=8,fig.path="Plots/",dev=c('png','pdf')}
...
write.xlsx(plot_data,file="Results/IPCC_plot_data.xlsx",sheetName = "Total_GHGs_Income")

```

### Total GHGs, split by sector

```{r ghgs_5sectors, echo=FALSE,warning=FALSE,fig.width=8,fig.path="Plots/",dev=c('png','pdf')}
...
plot_data <- plot_data %>% 
  ...
  mutate(IPCC_AR5_chapter=ifelse(is.na(IPCC_AR5_chapter),"NA",IPCC_AR5_chapter))
...
data <- gather(data,Year,value,-source,-gas) %>% 
  ...
  mutate(IPCC_AR5_chapter="CO2 Landuse") %>% 
...
plot_data$IPCC_AR5_chapter <- factor(plot_data$IPCC_AR5_chapter,levels(plot_data$IPCC_AR5_chapter)[c(4,5,7,3,1,2,6)])

plot_data %>%
  ggplot(.,aes(x=Year,y=total_ghg,fill=IPCC_AR5_chapter)) +
  geom_area() +
  ylab("GHG emissions (GtCO2eq/yr)") +
  theme_bw() +
  theme(axis.title.x = element_blank(),
...
xlsx::write.xlsx(plot_data,file="Results/IPCC_plot_data.xlsx",sheetName = "Total_GHGs_Sector",append=T)
...
xlsx::write.xlsx(plot_data,file="Results/IPCC_plot_data.xlsx",sheetName = "Total_GHG_gas",append=T)

# z <- spread(data,gas,value) %>%
...
xlsx::write.xlsx(budget_data,file="Results/IPCC_plot_data.xlsx",sheetName = "CO2_budget",append=T)

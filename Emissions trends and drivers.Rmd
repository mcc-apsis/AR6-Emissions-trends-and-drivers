...
plot_data <- edgar_GHG %>% 
  ...
  summarise(total_ghg=sum(GHG,rm.na=T)) 

plot_data %>% 
  ggplot(.,aes(x=Year,y=total_ghg/1e6,fill=WB.income)) +
  geom_area() +
...
data <- gather(data,Year,value,-source,-gas) %>% 
  ...
  mutate(IPCC_AR5_chapter="Land use") %>% 
...
plot_data$IPCC_AR5_chapter <- factor(plot_data$IPCC_AR5_chapter,levels(plot_data$IPCC_AR5_chapter)[c(2,3,5,1,4)])

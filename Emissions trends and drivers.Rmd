...
blarg <- edgar_GHG %>% 
  filter(ISO=="USA",Year==2017) %>% 
  select(IPCC.detailed,IPCC.description,Sector,Chapter)
xlsx::write.xlsx(blarg,file='Data/missing_designations.xlsx')
...
edgar_emissions_plot <- function(time_start,sector,gas) {
  ...
  p4 <- ggarrange(p1,p3,p2,ncol=1,nrow=3,heights=c(2,1.2,2))
  return(p4)
}
...
```{r edgar_plots, echo=FALSE,warning=FALSE,fig.width=8,fig.height=12,fig.path="Plots/",dev=c('png','pdf')}
...
plot_industry_GHG <- edgar_emissions_plot(time_start,'Industry','GHG')
plot_industry_GHG

plot_transport_GHG <- edgar_emissions_plot(time_start,'Transport','GHG')
plot_transport_GHG

plot_buildings_GHG <- edgar_emissions_plot(time_start,'Buildings','GHG')
plot_buildings_GHG

plot_AFOLU_GHG <- edgar_emissions_plot(time_start,'AFOLU','GHG')
plot_AFOLU_GHG

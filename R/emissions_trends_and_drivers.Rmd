...
```{r waterfall, echo=FALSE,warning=FALSE,fig.width=10,fig.height=6,fig.path="../Results/Plots/",dev=c('png','pdf')}
...
openxlsx::writeData(wb, sheet = "gwp_waterfall_data", waterfall_plot, colNames = T, rowNames = F)

######### plot different gwp totals in a waterfall

waterfall_plot$gas <- as.factor(waterfall_plot$gas)
waterfall_plot$gas <-  factor(waterfall_plot$gas,levels(waterfall_plot$gas)[c(2,3,1,5,4)])

waterfall_plot <- waterfall_plot %>% 
  arrange(gas)

waterfall_plot <- gather(waterfall_plot,gwp,value,gwp_ar2:gwp_ar6)
...
waterfall_plot <- left_join(waterfall_plot,uncertainties,by=c("gas"="gas"))
waterfall_plot <- waterfall_plot %>% 
  group_by(gwp) %>% 
...
p_waterfall <- waterfall_plot %>%
  ...
  geom_text(data=waterfall_plot %>% filter(gwp!="gwp_ar4") %>%  filter(gas=="CO2 Land use"),aes(x=gas,y=62,label=paste(gwp,"\n",round(total,1),  "Gt")),size=3.5,colour="#252525")+
...
p <- ggarrange(p_gas_totals,p_waterfall,widths=c(0.7,0.3),align="v")
p <- ggarrange(p,p_gas_trends,nrow=2,heights=c(0.7,0.3))
p

...
ar5_waterfall <- waterfall_plot(plot_data,gwps,"gwp_ar5",uncertainties)
ar6_waterfall <- waterfall_plot(plot_data,gwps,"gwp_ar6",uncertainties)
ar2_waterfall <- waterfall_plot(plot_data,gwps,"gwp_ar2",uncertainties)
...
gcb <- openxlsx::read.xlsx('../Data/CDIAC_fossilfuel_updated_GCP2019.xlsx',sheet="CDIAC CO2",rows=3:271,cols=1:9)
...
side_plot_data <- side_plot_data %>% 
  mutate(`GHG Emissions per capita (tCO2/cap)` = GHG/pop_UN) %>% 
  mutate(`GHG Emissions per gdp (gCO2/$)` = GHG/gdp_ppp_WB*1000) %>% 
  mutate(`GHG Emissions per gdp (gCO2/$)` = ifelse(year==2018,NA,`GHG Emissions per gdp (gCO2/$)`))

side_plot_data <- gather(side_plot_data,key,value,`GHG Emissions per capita (tCO2/cap)`:`GHG Emissions per gdp (gCO2/$)`)

p2 <- side_plot_data %>% 
  ggplot(.,aes(x=year,y=value,colour=region_ar6_5_short)) +
  geom_line() +
  theme_bw() +
...
load('../Data/basic.RData')
decomp <- decomp_figure_countries(1990,'GHG',edgar_GHG_ar5,basic)
...
plot <- ggarrange(p1 + theme(legend.position="none"),p2,ncol=2,nrow=1,widths=c(0.7,0.3))
plot <- ggarrange(plot,p3,p4,ncol=1,nrow=3,heights=c(0.3,0.35,0.35))
plot
...
```{r Fig9_sectors, echo=FALSE,warning=FALSE,fig.width=8,fig.height=12,fig.path="../Results/Plots/",dev=c('png','pdf')}
...
decomp <- decomp_figure_sectors(2010,'GHG',edgar_GHG_ar5)


  
p2 <- ggarrange(decomp$rate_plot,decomp$abs_plot,ncol=1,nrow=2)

plot <- ggarrange(p1,p2,ncol=1,nrow=2,heights=c(0.3,0.7))
plot

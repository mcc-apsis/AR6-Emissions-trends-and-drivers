...
### Waterfall plots for different GWPs

```{r waterfall, echo=FALSE,warning=FALSE,fig.width=4,fig.height=4,fig.path="../Results/Plots/",dev=c('png','pdf')}
...
openxlsx::addWorksheet(wb,"gwp_waterfall_data")
openxlsx::writeData(wb, sheet = "gwp_waterfall_data", waterfall_data, colNames = T, rowNames = F)
...
```{r gas_trend, echo=FALSE,warning=FALSE,fig.width=6,fig.height=4}
...
```{r ghgs_no_gwp, echo=FALSE,warning=FALSE,fig.width=8,fig.height=2,fig.path="../Results/Plots/",dev=c('png','pdf')}


plot_data_nogwp <- plot_data %>% 
  filter(gas!="Fgas")
  
plot_data_nogwp <- left_join(plot_data_nogwp,gwps %>% select(gas,gwp_ar6),by=c("gas"="gas"))

plot_data_nogwp <- plot_data_nogwp %>% 
  mutate(gwp_ar6=ifelse(grepl("CO2",gas),1,gwp_ar6)) %>% 
...
plot_data_nogwp <- plot_data_nogwp %>% 
  mutate(high = value+((uncertainty*value)/2)) %>% 
  mutate(low = value-((uncertainty*value)/2)) %>% 
  select(-uncertainty)
...
p_gas_trends <- plot_data_nogwp %>%
  ...
  theme_bw() +
  ylab('Emissions \n(original units noramlised to 1990)') +
...
p <- ggarrange(p_gas_totals,p_waterfall,widths=c(0.7,0.3),align="hv")
p <- ggarrange(p,p_gas_trends,nrow=2,heights=c(0.7,0.3),align="hv")
p

...
```{r crazy_countries,fig.width=8,fig.height=20,fig.path="../Results/Plots/testing/",dev=c('png','pdf')}
...
```{r cement,fig.width=8,fig.height=10,fig.path="../Results/Plots/testing/",dev=c('png','pdf')}
...
global_totals <- edgar_GHG_ar5 %>% 
  filter(year==2018) %>% 
  summarise_at(vars(CO2:GHG),sum,na.rm=T) %>% 
  mutate(chapter_title='Total')

totals_data <- edgar_GHG_ar5 %>% 
  filter(year==2018) %>% 

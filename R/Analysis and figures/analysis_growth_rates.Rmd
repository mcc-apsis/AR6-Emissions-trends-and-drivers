...
regions <- left_join(regions,regions_land,by = c("region_ar6_10", "year"))
regions <- regions %>% 
  mutate(value=ghg+land) %>% 
  mutate(value=ifelse(region_ar6_10=="Intl. Aviation",ghg,value)) %>% 
  mutate(value=ifelse(region_ar6_10=="Intl. Shipping",ghg,value)) %>% 
  mutate(value=value/1e9) %>% 
  mutate(category="Regions") %>% 
  mutate(subcategory="IPCC 6 regions")

regions <- regions %>% 
  select(category,subcategory,year,var=region_ar6_10,value)
...
sectors_indirect <- left_join(sectors_indirect,sectors_indirect_land,by = c("sector_title", "year"))

## add indirect CO2
sectors_indirect_indirect <- indirect_CO2_world %>% 
  group_by(year,sector_title) %>% 
  summarise(CO2_indirect=sum(CO2_indirect,na.rm=TRUE)*1e9) %>% 
  mutate(year=as.numeric(year))

sectors_indirect <- left_join(sectors_indirect,sectors_indirect_indirect,by = c("sector_title", "year"))

sectors_indirect <- sectors_indirect %>% 
  mutate(ghg=ifelse(sector_title=="AFOLU",ghg+land,ghg)) %>% 
  mutate(ghg=ifelse(sector_title=="Industry",ghg+CO2_indirect,ghg)) %>% 
  mutate(ghg=ifelse(sector_title=="Buildings",ghg+CO2_indirect,ghg)) %>% 
  mutate(ghg=ifelse(sector_title=="Transport",ghg+CO2_indirect,ghg)) %>% 
  mutate(value=ghg/1e9) %>% 
  mutate(category="Sectors (incl. indirect CO2)") %>% 
  mutate(subcategory=NA) %>% 
  select(category,subcategory,year,var=sector_title,value)
...
sectors <- left_join(sectors,sectors_land,by = c("sector_title", "year"))
...
subsectors <- edgar_ghg %>% 
  ...
  select(year,category,subcategory=sector_title,var=subsector_title,value)
...
saveWorkbook(wb,paste0("../../Results/Analysis/ipcc_ar6_data_growth_rates.xlsx"),overwrite=T)

save(rates,file = "../../Data/data_rates.RData")

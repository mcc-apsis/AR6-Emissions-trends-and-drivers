...
```{r all_countries_trend,echo=FALSE,warning=FALSE,fig.width=12,fig.height=14,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
cluster_subsectors <- plot_data %>% ggplot(.,aes(x=subsector_title,y=GHG_peak,fill=chapter_title)) + 
  ...
  scale_x_discrete(labels = function(subsector_title) str_wrap(subsector_title, width = 30)) +
  scale_fill_brewer(palette="Set2") +
  coord_flip() +
...
p1 <- rolling_rates %>% ggplot(.,aes(x=country,y=rate_rolling_CO2)) +
  ...
  theme(axis.text.x=element_blank(),
        ...
        strip.text = element_blank(),
        plot.margin = unit(c(0,0,0,0),units="cm")) +
  ggtitle(str_wrap("a. CO2 growth rates since peak years (5 year rolling avg.)",width=35))
...
p2 <- rolling_rates %>% ggplot(.,aes(x=country,y=year,fill=threshold_CO2)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        plot.margin = unit(c(0,0,0,0),units="cm")) +
  ggtitle(str_wrap("c. CO2 growth rates since peak years (5 year rolling avg. thresholds)",width=38))

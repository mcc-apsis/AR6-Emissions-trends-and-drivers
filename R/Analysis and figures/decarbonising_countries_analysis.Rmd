...
```{r all_countries_trend_co2,echo=FALSE,warning=FALSE,fig.width=12,fig.height=14,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}

## set facet labels and order by cluster and growth rate

data <- data %>%
  mutate(labels=paste0(country,"\n(",round(rate_CO2_peak,1),"%/yr, -",round(absolute_reduction_CO2_peak/1e6),"MtCO2)"))
...
plot_data <- gather(data,paths,value,CO2,GHG,CO2_cons,fit_CO2)
plot_data$paths <- as.factor(plot_data$paths)
plot_data$paths <- factor(plot_data$paths,levels=levels(plot_data$paths)[c(1,2,4,3)])

decarbonising_countries <- plot_data %>%
...
data <- data %>%
  mutate(labels=paste0(country,"\n(",round(rate_GHG_peak,1),"%/yr, -",round(absolute_reduction_GHG_peak/1e6),"MtCO2eq)"))
  #mutate(labels=paste0(country," (",round(rate_CO2_peak,1),"%/yr)"))
...
data$labels=fct_reorder(data$labels,data$rate_GHG_peak)
data$labels=fct_reorder2(data$labels,data$rate_GHG_peak,desc(as.numeric(data$cluster)))

plot_data <- gather(data,paths,value,CO2,GHG,CO2_cons,fit_GHG)
plot_data$paths <- as.factor(plot_data$paths)
plot_data$paths <- factor(plot_data$paths,levels=levels(plot_data$paths)[c(1,2,4,3)])
...
```{r all_countries_trend_lulucf,echo=FALSE,warning=FALSE,fig.width=10,fig.height=11,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
data <- data %>%
  mutate(labels=paste0(country," (",round(rate_GHG_land_peak,1),"%/yr)"))
...
data$labels=fct_reorder(data$labels,data$rate_GHG_land_peak)
data$labels=fct_reorder2(data$labels,data$rate_GHG_land_peak,desc(as.numeric(data$cluster)))

plot_data <- gather(data,paths,value,CO2,GHG,CO2_cons,GHG_land,fit_GHG_land)
plot_data$paths <- as.factor(plot_data$paths)
plot_data$paths <- factor(plot_data$paths,levels=levels(plot_data$paths)[c(1,2,4,5,3)])
...
```{r cluster_summary,echo=FALSE,warning=FALSE,include=FALSE,dpi=300,fig.width=8,fig.height=2}
...
```{r map,echo=FALSE,warning=FALSE,include=FALSE,dpi=300,fig.width=8,fig.height=3}
...
```{r cluster_stats,echo=FALSE,warning=FALSE,dpi=300,fig.width=8,fig.height=2,include=FALSE}
...
```{r cluster_abs_rel_reductions,echo=FALSE,warning=FALSE,dpi=300,include=FALSE,fig.width=6,fig.height=2}
...
p4 <- plot_data %>% ggplot(.,aes(x=var,y=value,fill=cluster)) +
  ...
  theme(legend.position = "none",
        ...
        strip.background = element_blank(),
        strip.text = element_blank()) +
  ggtitle("d. Relative reduction in annual emissions since peak years (%)")
p4 

```

```{r cluster_plot,echo=FALSE,warning=FALSE,fig.width=6.5,dpi=300,fig.height=8,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
country_clusters <- p2 / p5 / p1 / p3 / p4 + plot_layout(heights = c(2,0.2,1,1,1))
country_clusters

```

```{r country_rates,echo=FALSE,warning=FALSE,fig.width=8,fig.height=7,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
annotes = data.frame(cluster=c("1","2","3"),cluster_name=c("Recent peak","Long-term decline","Former Eastern Bloc"),country=c("Netherlands","Belgium","Slovakia"))
...
r4 <- rate_data %>% 
  ...
  theme(axis.title.y = element_blank(),
        ...
        strip.text = element_blank(),
        plot.margin = unit(c(0,0,0,0),units="cm")) +
  ggtitle("Average annual reduction rate since peak (%/yr)")

country_rates <- r1 + r2 + r3 + r4 + plot_layout(widths = c(1,1.5,1.5,6))
country_rates
...
```{r country_sectors_GHG,echo=FALSE,warning=FALSE,fig.width=6.5,fig.height=7,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
annotes <- data.frame(cluster=c("1","2","3"),country=c("Cyprus","North Macedonia","Slovakia"),cluster_name=c("Recent peak","Long-term decline","Former Eastern Bloc"))
...
```{r calc_subsectors_clusters,echo=FALSE,warning=FALSE,fig.width=7,fig.height=6,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
```{r cluster_subsectors_GHG,echo=FALSE,warning=FALSE,fig.width=9,fig.height=8,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}


plot_data <- subsector_data_clusters %>% 
  #filter(GHG_2018>15) %>% 
  filter(subsector_title!="Biomass burning (CO2, CH4) (Agriculture)") %>% 
  filter(subsector_title!="Biomass energy systems (Energy systems)") %>% 
  filter(subsector_title!="Non-CO2 (all buildings) (Buildings)") %>% 
  filter(subsector_title!="Rice cultivation (CH4) (Agriculture)") %>% 
...
cluster_subsectors <- plot_data %>% ggplot(.,aes(x=subsector_title,y=GHG_peak,fill=chapter_title)) + 
  ...
  scale_x_discrete(labels = function(subsector_title) str_wrap(subsector_title, width = 32)) +
  scale_fill_brewer(palette="Set2") +
  coord_flip() +
...
rolling_rates <- left_join(rolling_rates,cluster_data %>% select(country,cluster,peak_year),by = "country")
...
scenario_data <- scenario_data %>% 
  mutate(aggregated_category=ifelse(aggregated_category=="1.5C","1.5oC warming (>50% probability)",aggregated_category)) %>% 
...
```{r rates_over_time,echo=FALSE,warning=FALSE,fig.width=7,fig.height=9,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}

rolling_rates <- rolling_rates %>% 
  mutate(threshold_CO2=ifelse(rate_rolling_CO2>-1,"> -1%/yr","< -1%/yr")) %>% 
  mutate(threshold_CO2=ifelse(rate_rolling_CO2>-2,threshold_CO2,"< -2%/yr")) %>% 
  mutate(threshold_CO2=ifelse(rate_rolling_CO2>-3,threshold_CO2,"< -3%/yr")) %>% 
  mutate(threshold_CO2=ifelse(rate_rolling_CO2>-4,threshold_CO2,"< -4%/yr"))
...
rolling_rates$country <- as.factor(rolling_rates$country)
rolling_rates$country <- fct_reorder(rolling_rates$country,desc(rolling_rates$median))


p1 <- rolling_rates %>% ggplot(.,aes(x=country,y=rate_rolling_CO2)) +
  geom_hline(yintercept=0) +
  ...
  theme(axis.text.x=element_blank(),
        ...
        strip.text = element_blank(),
        plot.margin = unit(c(0,0,0,0),units="cm")) +
  ggtitle(str_wrap("a. CO2 reduction rates since peak years (5 year rolling avg.)",width=35))
...
p3 <- scenario_data %>% ggplot(.,aes(x=aggregated_category,y=rate,fill=aggregated_category)) +
  ...
  theme(axis.title.y = element_blank(),
        ...
        legend.position="none",
        panel.grid.minor = element_blank()) +
  ggtitle(str_wrap("b. Average CO2 reduction rates in scenarios compatible with warming targets",width=40))

p2 <- rolling_rates %>% ggplot(.,aes(x=country,y=year,fill=threshold_CO2)) +
  geom_tile(color="#cccccc",size=0.2) +
  #scale_fill_manual(values=c("#bdbdbd","#969696","#636363","#252525"))+
  scale_fill_manual(values=c("#ffffcc","#a1dab4","#41b6c4","#2c7fb8","white"))+
  coord_flip()  +
  facet_grid(cluster~.,scales="free",space="free") +
  scale_y_continuous(breaks=c(1975,1985,1995,2005,2015)) +
  labs(fill = "Rolling average growth rate") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        panel.grid.minor = element_blank(),
        plot.title = element_text(size = 11),
        ...
        axis.ticks.y = element_blank(),
        plot.margin = unit(c(0,0,0,0),units="cm")) +
  ggtitle(str_wrap("c. CO2 reduction rates since peak years (5 year rolling avg. thresholds)",width=38))

country_rolling_rates <- p1 + p2 + p3 + guide_area() + plot_layout(heights=c(6,1)) + plot_layout(guides = 'collect')
country_rolling_rates

```

```{r gases,echo=FALSE,warning=FALSE,fig.width=7,fig.height=8,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
```{r kaya,echo=FALSE,warning=FALSE,fig.width=7,fig.height=8,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}

load('../../Data/kaya/kaya.RData')
kaya_data <- kaya_data %>% 
  mutate(name=ifelse(name=="Slovak Republic","Slovakia",name)) %>% 
...
ipcc_sectors <- ipcc_sectors %>% 
  select(EDGAR_code=code,description,sector=IPCC_AR6_chapter_title,subsector=subsector_title)

openxlsx::addWorksheet(wb,"sectors")
openxlsx::writeData(wb, sheet = "sectors", ipcc_sectors, colNames = T, rowNames = F)
...
openxlsx::addWorksheet(wb,"gwps")
openxlsx::writeData(wb, sheet = "gwps", gwps, colNames = T, rowNames = F)
...
all_data <- left_join(edgar_GHG_ar6,data %>% select(country,cluster,peak_year) %>% distinct(),by = "country")
all_data <- all_data %>% filter(!is.na(cluster))
all_data <- all_data %>% 
  group_by(ISO,country,cluster,year,peak_year,sector=chapter_title,subsector_title) %>% 
  summarise_at(vars(CO2,CH4,N2O,Fgas,GHG),sum,na.rm=TRUE)

openxlsx::addWorksheet(wb,"all_data")
openxlsx::writeData(wb, sheet = "all_data", all_data, colNames = T, rowNames = F)
...
```{r countries,echo=FALSE,warning=FALSE,fig.width=7,fig.height=8,dpi=300,fig.path="../../Results/Plots/Decarbonising countries/countries/",dev=c('png','pdf')}

...
```{r clustering,echo=FALSE,warning=FALSE,fig.width=10,fig.height=11,fig.path="../../Results/Plots/Peak/",dev=c('png','pdf')}
...
table <- data %>% 
  filter(year==2018) %>% 
  summarise(cluster=nth(cluster,1),peak_year=nth(peak_year,1),CO2_rate=nth(rate_CO2_peak,1),GHG_rate=nth(rate_GHG_peak,1),CO2_2018=nth(CO2,1),GHG_2018=nth(GHG,1),pop_2018=nth(pop_UN,1),CO2_peak=nth(CO2_peak,1),GHG_peak=nth(GHG_peak,1),abs_reduction_CO2=nth(absolute_reduction_CO2_peak,1),abs_reduction_GHG=nth(absolute_reduction_GHG_peak,1),rel_reduction_CO2=nth(percent_reduction_CO2_peak,1),rel_reduction_GHG=nth(percent_reduction_GHG_peak,1))
...
```{r abs_rel_reductions,warning=FALSE,fig.width=6,fig.height=2}
...
```{r cluster_plot,echo=FALSE,warning=FALSE,fig.width=6.5,fig.height=8,fig.path="../../Results/Plots/Peak/",dev=c('png','pdf')}
...
```{r rate_plots,echo=FALSE,warning=FALSE,fig.width=6,fig.height=7,fig.path="../../Results/Plots/Peak/",dev=c('png','pdf')}

r1 <- data %>% 
  filter(year==2018) %>% 
  ggplot(.,aes(y="a",x=reorder(country,desc(rate_CO2_peak)),label=peak_year))+
  ...
  theme(axis.title.y = element_blank(),
        ...
        strip.background = element_blank(),
        strip.text = element_blank()) +
  ggtitle("Country CO2 and GHG reduction rates since peak")
...
r1 + r2 + plot_layout(widths = c(1,6))
...
```{r sectors,echo=FALSE,warning=FALSE,fig.width=6,fig.height=2,fig.path="../../Results/Plots/Peak/",dev=c('png','pdf')}


sector_data <- right_join(edgar_GHG_ar6,data %>% filter(year==2018) %>% select(country,peak_year,cluster) %>% distinct(),by = "country")


# sector_data <- sector_data %>% 
#   group_by(country,cluster,year,chapter_title,subsector_title) %>% 
#   summarise_at(vars(CO2,GHG),sum,na.rm=TRUE)

sector_data_clusters <- sector_data %>% 
  filter(year %in% c(1970,1990,2010,2018)) %>%
  group_by(cluster,year,chapter_title) %>% 
  summarise_at(vars(CO2,GHG),sum,na.rm=TRUE)

sector_data_clusters <- sector_data_clusters %>% 
group_by(cluster) %>% 
  mutate(GHG=GHG/first(GHG)) %>% 
  mutate(year=as.factor(year))

sector_data_clusters %>% 

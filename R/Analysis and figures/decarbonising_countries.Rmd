...
sector_cluster_country_data <- sector_cluster_data
...
sector_cluster_data <- sector_cluster_data %>%
  group_by(cluster) %>% 
  mutate(abs_CO2_tot=sum(abs_CO2)) %>% 
  mutate(abs_GHG_tot=sum(abs_GHG))
...
```{r cluster_subsectors_CO2,echo=FALSE,warning=FALSE,fig.width=7,fig.height=6,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}


# order rows

plot_data <- sector_cluster_data %>% 
  filter(abs(abs_CO2_fraction)>2) %>% 
  arrange(cluster,abs_CO2)
...
annotes <- data.frame(cluster=c("1","2","3"),country=c(1,10,20))


plot_data %>% 
  #filter(abs(abs_CO2_fraction)>2) %>% 
  ggplot(.,aes(x=order,y=-abs_CO2,fill=chapter_title)) +
  geom_bar(stat='identity',color="#636363",width = 0.75) +
  geom_text(aes(x=order,y=-abs_CO2,label=paste0(ifelse(abs_CO2_fraction>0,"+",""),round(abs_CO2_fraction,1),"%"),hjust = ifelse(abs_CO2 <= 0, -0.1, 1.1)),size=3,colour="#636363",size=3.5) +
  geom_text(data=annotes,inherit.aes=FALSE,aes(x=country,y=-1000,label=paste0("Group ",cluster)),color="#636363",hjust=0,vjust=0,size=3.5,show.legend=FALSE) +
  coord_flip() +
  scale_x_continuous(
    breaks = plot_data$order,
    labels = plot_data$subsector_title
  ) +
  ylim(-1000,300) +
  ylab("Mt CO2") +
  ...
  ggtitle("CO2 Emissions change in subsectors by cluster since peak year")

```

```{r cluster_subsectors_GHG,echo=FALSE,warning=FALSE,fig.width=7.5,fig.height=7,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}


# filter to > 2% and order rows

plot_data <- sector_cluster_data %>% 
  filter(abs(abs_GHG_fraction)>2) %>% 
  arrange(cluster,abs_GHG)

plot_data$order <- 1:length(plot_data$cluster)

plot_data$subsector_title <- fct_reorder(plot_data$subsector_title,plot_data$order)

annotes <- data.frame(cluster=c("1","2","3"),country=c(1,15,25))

plot_data %>% 
  #filter(abs(abs_CO2_fraction)>2) %>% 
  ggplot(.,aes(x=order,y=-abs_GHG,fill=chapter_title)) +
  geom_bar(stat='identity',color="#636363",width = 0.75) +
  geom_text(aes(x=order,y=-abs_GHG,label=paste0(ifelse(abs_GHG_fraction>0,"+",""),round(abs_GHG_fraction,1),"%"),hjust = ifelse(abs_GHG <= 0, -0.1, 1.1)),size=3,colour="#636363",size=3.5) +
  geom_text(data=annotes,inherit.aes=FALSE,aes(x=country,y=-1000,label=paste0("Group ",cluster)),color="#636363",hjust=0,vjust=0,size=3.5,show.legend=FALSE) +
  coord_flip() +
  scale_x_continuous(
    breaks = plot_data$order,
  ...
  theme(axis.title.y = element_blank(),
        ...
        strip.text = element_blank(),
        panel.grid.minor.y = element_blank()) +
  ggtitle("GHG Emissions change in subsectors by cluster since peak year")

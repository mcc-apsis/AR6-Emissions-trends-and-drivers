...
blarg <- data %>% select(country,fsi,fsi_rank) %>% distinct()
...
data %>% 
  ...
  scale_x_continuous(limits=c(1970,2019),breaks=c(1970,1990,2010)) +
...
```{r all_countries_trend_with_consumption,echo=FALSE,warning=FALSE,fig.width=10,fig.height=11,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}

data %>% 
  ggplot(.,aes(x=year,y=CO2)) +
  geom_path() +
  geom_line(data=data,aes(x=year,y=fit),colour="red") +
  geom_path(data=data,aes(x=year,y=CO2_cons,color="#2c7fb8")) +
  geom_text(data=data %>% filter(year==2015),aes(x=1970,y=0,label=paste0("Group ",cluster),color=cluster),hjust="inward",vjust="inward",size=3.5) + #color="#636363") + 
  scale_x_continuous(limits=c(1970,2019),breaks=c(1970,1990,2010)) +
  scale_y_continuous(expand = c(0.1, .1), limits = c(0, NA)) +
  facet_wrap(.~labels,scales="free",labeller = label_wrap_gen(width=19,multi_line = TRUE)) +
...
table <- table %>% 
  ...
  mutate(CO2_2019=CO2_2019/1e6) %>% 
  mutate(GHG_2019=GHG_2019/1e6) %>% 
  mutate(pop_2019=pop_2019/1e6) %>% 
  mutate(CO2_peak=CO2_peak/1e6) %>% 
  mutate(GHG_peak=GHG_peak/1e6) %>% 
  mutate(CO2_2019_pc = CO2_2019/pop_2019) %>% 
  mutate(GHG_2019_pc = GHG_2019/pop_2019)
...
totals <- table %>% 
  summarise(CO2_2019=sum(CO2_2019),GHG_2019=sum(GHG_2019),pop_2019=sum(pop_2019),
...
table <- table %>%
  mutate(GHG_fraction=(GHG_2019/world_GHG$GHG)*100) %>%
  mutate(CO2_fraction=(CO2_2019/world_GHG$CO2)*100) %>% 
  mutate(pop_fraction=(pop_2019/(world_pop$population/1e6))*100)
...
cluster_table <- table %>% 
  filter(!is.na(cluster)) %>% 
  group_by(cluster) %>% 
  summarise(mean_peak_year=round(mean(peak_year),0),CO2_2019=sum(CO2_2019),GHG_2019=sum(GHG_2019),pop_2019=sum(pop_2019),CO2_peak=sum(CO2_peak),GHG_peak=sum(GHG_peak),abs_reduction_CO2=sum(abs_reduction_CO2),abs_reduction_GHG=sum(abs_reduction_GHG),countries=paste0(country,collapse="; "))
...
r4 <- rate_data %>% 
  ...
  #scale_fill_manual(labels=c("CO2 cons emissions","CO2 emissions","GHG emissions"),values = c("#31a354","#d7191c","#2c7bb6")) +
  theme(axis.title.y = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.x = element_blank(),
...
```{r country_rates_cons,echo=FALSE,warning=FALSE,fig.width=8,fig.height=9,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}
...
r1 + r2 + r3 + r5 + plot_layout(widths = c(1,1.5,1.5,6))
...
sector_data <- right_join(edgar_GHG_ar6,data %>% filter(year==2019) %>% select(country,peak_year,cluster,absolute_reduction_CO2_peak,absolute_reduction_GHG_peak) %>% distinct(),by = "country")
...
sector_cluster_data <- right_join(edgar_GHG_ar6,data %>% filter(year==2019) %>% select(country,peak_year,cluster,absolute_reduction_CO2_peak,absolute_reduction_GHG_peak) %>% distinct(),by = "country")
...
sector_cluster_data <- sector_cluster_data %>% 
  ...
  summarise_at(vars(CO2_peak,CO2_2019,abs_CO2,GHG_peak,GHG_2019,abs_GHG),sum,na.rm=TRUE)
...
plot_data %>% ggplot(.,aes(x=subsector_title,y=GHG_peak,fill=chapter_title)) + 
  ...
  geom_bar(aes(x=subsector_title,y=GHG_2019),stat='identity',color="#636363",width = 0.75) +
  geom_text(aes(x=subsector_title,y=ifelse(GHG_peak>GHG_2019,GHG_peak,GHG_2019),label=paste0(ifelse(rel_GHG>0,"+",""),round(rel_GHG,1),"%")),hjust = -0.15,size=3,colour="#636363",size=3.5) +
  ...
  ggtitle("Subsector GHG emissions at peak vs 2019 by cluster")
...
```{r kaya,echo=FALSE,warning=FALSE,fig.width=8,fig.height=9,fig.path="../../Results/Plots/Decarbonising countries/",dev=c('png','pdf')}

load('../../Data/kaya/kaya.RData')

kaya_data <- left_join(kaya_data,data %>% select(ISO,cluster,peak_year) %>% distinct(),by = "ISO")
...
kaya_data_clusters <- kaya_data %>% 
  filter(name!="Lithuania") %>% # missing data after 1990
...
# 
# kaya_data_clusters %>%
#     ggplot(.,aes(x=year,y=value,group=factor,colour=factor)) +
...
#     #scale_colour_brewer(type = "qual",palette="Set2",labels=c("CO2 Emissions","CO2/Energy","Energy","Energy/GDP","GDP/Population","Population")) +     #scale_x_continuous(breaks=c(1990,2000,2010,2018),limits=c(1990,2023.5)) + 
...
#     geom_vline(xintercept=c(time_start,2019),alpha=0.3,linetype="dashed") +
...
#   
#   total_GHG <- data %>% filter(year==2019) %>% group_by(country) %>% summarise(GHG=sum(GHG))
#   
#   data <- data %>% 
#     filter(year==2019) %>% 
...
#     ylab(paste0("GHG Emissions change in major sectors, ",time_start,"-2019 (MtCO2eq)")) +
#     xlab('') +
#     coord_flip() +
#     theme(legend.position="none")
#   

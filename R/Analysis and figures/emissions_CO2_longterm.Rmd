...
cumulative_CO2 <- cumulative_CO2 %>% 
  ...
  add_row(key="1.5 budget (2020-)",value=382-43) %>% 
  add_row(key="2.0 budget (2020-)",value=915-43) # subtract 2019 total CO2 emissions

# from SPM_TS_table_CH3_template_filled20210827_C19.xlsx
...
cumulative_CO2 <- cumulative_CO2 %>% 
  mutate(start = ifelse(key=="2.0 budget (2020-)",start-(382-43),start)) %>% 
  mutate(end = ifelse(key=="2.0 budget (2020-)",end-(382-43),end))
...
cumulative_CO2 <- cumulative_CO2 %>%
  mutate(lower_budget=ifelse(key=="1.5 budget (2020-)",start+271-43,NA)) %>%
  mutate(higher_budget=ifelse(key=="1.5 budget (2020-)",start+575-43,NA)) %>%
  mutate(lower_budget=ifelse(key=="2.0 budget (2020-)",start+630-43,lower_budget)) %>%
  mutate(higher_budget=ifelse(key=="2.0 budget (2020-)",start+1170-43,higher_budget))


# add additional budget uncertainty of +-250 GtCO2 (dont do this any more as we switched to WG3 budgets)
# cumulative_CO2 <- cumulative_CO2 %>% 
#   mutate(lower_uncertainty=ifelse(id>2,end-250,NA)) %>% 
#   mutate(higher_uncertainty=ifelse(id>2,end+250,NA))

cumulative_CO2 <- cumulative_CO2 %>%
  mutate(key=ifelse(key=="1.5 budget (2020-)","1.5\u00B0C budget (C1)",key)) %>%
  mutate(key=ifelse(key=="2.0 budget (2020-)","2\u00B0C budget (C3)",key))

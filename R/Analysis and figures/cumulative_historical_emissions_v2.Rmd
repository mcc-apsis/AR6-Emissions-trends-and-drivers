---
title: "Cumulative historical emissions"
...
# gcb_regions<-full_join(ipcc_regions,gcb_countries,by=c("country_name"="V2"))
# 
# GCB<-gather(GCB[,c(1:218,232)],country,ghg,Afghanistan:Bunkers)#Zimbabwe)
# 
# GCB<-left_join(GCB,gcb_regions,by=c("country"="V1"))
# 
# GCB<-GCB %>%
#   mutate(ghg=(ghg*44/12)/1e3) %>% #the data provided by GCB are in MtC (1MtC = 1 million tonne of carbon = 3.664 million tonnes of CO2)
#   mutate(region_ar6_6=ifelse(country=="Bunkers","Int. Aviation and Shipping",region_ar6_6)) %>%
#   mutate(region_ar6_10=ifelse(country=="Bunkers","Int. Aviation and Shipping",region_ar6_10))
# 
# ussr_shares<-GCB %>%
...
# 
# #cdiac_countries<-cbind(cdiac_countries,NA)
gcb<-full_join(ipcc_regions,gcb_hist,by=c("ISO"="ISO 3166-1 alpha-3"))
...
gcb_hist<-gcb_hist %>%
  ...
  mutate(`ISO 3166-1 alpha-3`=ifelse(Country=="International Transport","",`ISO 3166-1 alpha-3`))
...
land_1850<-land %>%
  group_by(region_ar6_10) %>%
  summarise(ghg_cum_1850=sum(mean,na.rm=TRUE)/1e9) %>%
  mutate(source="CO2-LULUCF")
  
hist_emissions_1850<-rbind(hist_emissions_1850,land_1850)

hist_emissions_1850<-hist_emissions_1850 %>%
  ungroup() %>%
  mutate(share=ghg_cum_1850/sum(ghg_cum_1850)*100)

wb <- openxlsx::createWorkbook(title = paste("cumulative_historical_emissions_by_region"))

openxlsx::addWorksheet(wb,"emissions 1850-2019 10 regions")
openxlsx::addWorksheet(wb,"emissions 1750-2019 10 regions")

openxlsx::writeData(wb,"emissions 1850-2019 10 regions",hist_emissions_1850, colNames = T, rowNames = F)
openxlsx::writeData(wb,"emissions 1750-2019 10 regions",hist_emissions_1750, colNames = T, rowNames = F)

```

```{r plot_region_10,echo=FALSE}
hist_emissions_1850 %>%
  ggplot(aes(x=source,y=ghg_cum_1850,fill=region_ar6_10))+
...
blarg <- hist_emissions_1850 %>% 
  group_by(region_ar6_10) %>%
  summarise_at(vars(ghg_cum_1850,share),sum,na.rm=TRUE) %>%
  ungroup() %>%
  arrange(desc(region_ar6_10)) %>% 
  mutate(label=paste0(region_ar6_10," (",round(share,1),"%)")) %>%
  #arrange(desc(chapter_title),GHG) %>% 
  ungroup() %>% 
  mutate(label_position = cumsum(ghg_cum_1850) - ghg_cum_1850/2)

hist_emissions_1850 %>%
  ggplot(aes(x=01,y=ghg_cum_1850,fill=region_ar6_10))+
  geom_bar(stat="identity",position = "stack")+
...
hist_emissions_1750 %>%
  ggplot(aes(x=source,y=ghg_cum_1750,fill=region_ar6_10))+
  geom_bar(stat="identity",position = "stack")+
  ylab('CO2 Emissions (Gt)') +
  xlab('') +
...
hist_emissions_1750<-left_join(hist_emissions_1750,ussr_shares,by="region_ar6_6")
hist_emissions_1750<-left_join(hist_emissions_1750,yugoslavia_shares,by="region_ar6_6")

hist_emissions_1750<-hist_emissions_1750 %>%
  group_by(region_ar6_6) %>%
  mutate(ghg_cum_1750=sum(ghg_cum_1750,ghg_ussr_cum_1750,ghg_yugo_cum_1850,na.rm = TRUE)) %>%
  select(region_ar6_6,ghg_cum_1750) %>%
  mutate(source="CO2-FFI")

hist_emissions_1850<-left_join(hist_emissions_1850,ussr_shares,by="region_ar6_6")
hist_emissions_1850<-left_join(hist_emissions_1850,yugoslavia_shares,by="region_ar6_6")

hist_emissions_1850<-hist_emissions_1850 %>%
  group_by(region_ar6_6) %>%
  mutate(ghg_cum_1850=sum(ghg_cum_1850,ghg_ussr_cum_1850,ghg_yugo_cum_1850,na.rm = TRUE)) %>%
  select(region_ar6_6,ghg_cum_1850) %>%
  mutate(source="CO2-FFI")
...
openxlsx::writeData(wb,"emissions 1850-2019 6 regions",hist_emissions_1850, colNames = T, rowNames = F)
openxlsx::writeData(wb,"emissions 1750-2019 6 regions",hist_emissions_1750, colNames = T, rowNames = F)

openxlsx::saveWorkbook(wb,"../../Results/Plot data/cumulative_historical_emissions_by_region.xlsx",overwrite=T)

```

```{r plot_6_regions,echo=FALSE}
hist_emissions_1850 %>%
  ggplot(aes(x=source,y=ghg_cum_1850,fill=region_ar6_6))+
  geom_bar(stat="identity",position = "stack")+
  ylab('CO2 Emissions (Gt)') +
  xlab('') +
  ggtitle("Cumulative CO2 emissions 1850-2019")+
...
hist_emissions_1850 %>%
  ggplot(aes(x=1,y=ghg_cum_1850,fill=region_ar6_6))+
  geom_bar(stat="identity",position = "stack")+
...
hist_emissions_1750 %>%
  ggplot(aes(x=source,y=ghg_cum_1750,fill=region_ar6_6))+
  geom_bar(stat="identity",position = "stack")+
  ylab('CO2 Emissions (Gt)') +
  xlab('') +
  ggtitle("Cumulative CO2 emissions 1750-2019")+

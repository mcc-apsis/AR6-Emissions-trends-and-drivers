...
co2 <- rbind(co2,co2_dev %>% filter(region_ar6_dev=="ldc") %>% rename(region_ar6_6=region_ar6_dev))
...
pop_dev <- left_join(ipcc_regions,wdi_data_gdp_pop %>% select(ISO=iso3c,year,population), by = "ISO")
...
pop <- rbind(pop,pop_dev %>% filter(region_ar6_dev=="ldc") %>% rename(region_ar6_6=region_ar6_dev))
...
co2_cons <- rbind(co2_cons,co2_cons_dev %>% filter(region_ar6_dev=="ldc") %>% rename(region_ar6_6=region_ar6_dev))
...
ghg <- rbind(ghg,ghg_dev %>% filter(region_ar6_dev=="ldc") %>% rename(region_ar6_6=region_ar6_dev))
...
ghg_lulucf <- left_join(ghg_lulucf,co2_land %>% select(region_ar6_6,co2_land=value),by = "region_ar6_6")
...
co2_1850_ex <- rbind(co2_1850_ex,co2_1850_dev %>% filter(region_ar6_dev=="ldc") %>% rename(region_ar6_6=region_ar6_dev))
...
data <- data %>% 
  ...
  add_row(region_ar6_6="ldc",var="Territorial CO2, incl. LULUCF (1850-2019)",category="b. Historical cumulative emissions by region") %>% 
  add_row(region_ar6_6="ldc",var="Territorial GHG, incl. LULUCF (2019)",category="a. Current population and emissions by region")



data$var <- as.factor(data$var)
data$category <- as.factor(data$category)
data$var <- fct_relevel(data$var,"Population (2019)","Consumption CO2, fossil only (2018)","Territorial CO2, fossil and proccess (2019)","Territorial GHG, excl. LULUCF (2019)","Territorial GHG, incl. LULUCF (2019)","Territorial CO2, excl. LULUCF (1850-2019)","Territorial CO2, incl. LULUCF (1850-2019)")
...
writeData(wb, sheet = "top panels", data %>% filter(region_ar6_6!="ldc"), colNames = T, rowNames = F)
writeData(wb, sheet = "lower panels", data_dev %>% filter(region_ar6_6=="ldc"), colNames = T, rowNames = F)
...
```{r equity_perspectives,dpi=300,fig.width=10,fig.height=5,fig.path="../../Results/Plots/",dev=c('png','pdf')}

p1 <- data %>%
  filter(region_ar6_6!="ldc") %>% 
  ggplot(.,aes(x=var,y=fraction,fill=region_ar6_6)) +
  geom_bar(stat='identity',color="#737373") +
  geom_text(data=data %>% filter(region_ar6_6=="Africa"),aes(x=var,y=1.05,label=label),vjust=0,lineheight=0.75,color="#525252") +
...
p_labels <- data %>%
  ...
  ggplot(.,aes(x=1,y=position,label=str_wrap(region_ar6_6,width=15),color=region_ar6_6)) +
...
p_labels_2 <- data_dev %>%
  ...
  filter(var=="Territorial GHG, incl. LULUCF (2019)") %>%

...
load('../../Data/cumulative_historical_emissions_by_regions.RData')
...
gather_data <- function(region,ipcc_regions,edgar_ghg,land,wdi_data_gdp_pop,hist_emissions_1750,hist_emissions_1850) {
  ...
  co2_long_1750 <- hist_emissions_1750 %>% 
    rename_all(gsub, pattern = paste0(region,'$'), replacement = 'region_select') %>% 
    filter(region_select!="Int. Aviation and Shipping") %>% 
    rename(value=ghg_cum_1750) %>% 
    mutate(var="Territorial CO2, excl. LULUCF (1750-2019)") %>% 
    mutate(category="Historical cumulative emissions") %>% 
    select(-source)
  co2_long_1750$total <- sum(co2_long_1750$value)
  co2_long_1750 <- co2_long_1750 %>% 
    mutate(fraction=value/total) %>% 
    mutate(label=paste0(round(total,0)," Gt CO2eq"))
  
  
  co2_long_1850 <- hist_emissions_1850 %>% 
    rename_all(gsub, pattern = paste0(region,'$'), replacement = 'region_select') %>% 
    filter(region_select!="Int. Aviation and Shipping") %>% 
    rename(value=ghg_cum_1850)
  
  co2_long_1850 <- co2_long_1850 %>% 
    group_by(region_select) %>% 
    summarise(value=sum(value))
  
  co2_long_1850$total <- sum(co2_long_1850$value)
  co2_long_1850 <- co2_long_1850 %>% 
    mutate(fraction=value/total) %>% 
    mutate(label=paste0(round(total,0)," Gt CO2eq")) %>% 
  ...
  data <- rbind(data,co2_long_1750)
  data <- rbind(data,co2_long_1850)
  
  
  data$var <- as.factor(data$var)
  data$category <- as.factor(data$category)
  data$var <- fct_relevel(data$var,"Population (2019)","Consumption CO2, fossil only (2018)","Territorial GHG, excl. LULUCF (2019)","LULUCF CO2 (2019)","Territorial GHG, incl. LULUCF (2019)")
...
data <- gather_data("region_ar6_6",ipcc_regions,edgar_ghg,land,wdi_data_gdp_pop,hist_emissions_1750,hist_emissions_1850)


##### find positions for each region and % value

data <- data %>% 
  arrange(desc(region_select)) %>% 
  group_by(var) %>% 
...
for (i in 1:length(unique(data$var))) {
  ...
  for (j in 2:(length(data$region_select[data$var==handle]))) {
...
wrap_elements(p2) / wrap_elements(p1)
...
#   
# p2 <- data %>% 
#   filter(var=="Territorial CO2, fossil and proccess (1970-2018)") %>% 
#   ggplot(.,aes(x=var,y=fraction,fill=region_select)) +
#   geom_bar(stat='identity',color="#737373") +
#   geom_text(data=data %>% 
#               filter(category=="Historical cumulative emissions") %>% 
...
#   filter(category=="Historical cumulative emissions") %>% 
#   filter(var!="Territorial CO2, fossil and proccess (1970-2018)") %>% 
#   filter(var!="LULUCF CO2 (1850-2019)") %>% 

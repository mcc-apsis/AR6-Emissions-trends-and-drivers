...
plot_data <- plot_data %>%
  mutate(region_ar6_10=ifelse(region_ar6_10=="Intl. Aviation","Intl. Shipping & Aviation",region_ar6_10)) %>% 
  mutate(region_ar6_10=ifelse(region_ar6_10=="Intl. Shipping","Intl. Shipping & Aviation",region_ar6_10))
...
plot_data$region_ar6_10 <- as.factor(plot_data$region_ar6_10)
plot_data$region_ar6_10 <- fct_relevel(plot_data$region_ar6_10,
...
line_data <- data.frame(x=c(1990,1990,2000,2000,2010,2010,2019,2019),y=c(0,61))
...
## adjust levels, as in SPM2
countries$region_ar6_10 <- as.factor(countries$region_ar6_10)
countries$region_ar6_10 <- fct_relevel(countries$region_ar6_10,
                                      "Intl. Shipping & Aviation",
                                      "Australia, Japan, New Zealand",
                                      "Middle East",
                                      "Eastern Europe and West-Central Asia",
...
colors = c("#66C2A5","#CF9C76","#CF948C","#969DCA","#D58EC4","#C6B18B","#B7D84C","#F6D832","#EFCC6B","#D5BE9D")


countries <- left_join(countries,wdi_data_gdp_pop %>% select(-country),by=c("ISO"="iso3c","year"="year"))
...
p2 <- countries %>%
  ...
  geom_bar(aes(y = avg_growth,fill=region_ar6_10),stat='identity',colour="#525252")+
...
p3 <- countries %>% 
  ...
  #ggplot(.,aes(x = reorder(country,value),y = value/1e9, fill=region_ar6_10)) +
  ggplot(.,aes(x = reorder(country,abs_growth)))+
  geom_bar(aes(y = abs_growth,fill=region_ar6_10),stat='identity',colour="#525252")+
...
p4 <- countries %>% 
  ...
  #ggplot(.,aes(x = reorder(country,value),y = value, fill=region_ar6_10)) +
  #geom_bar(stat='identity',colour="#737373") + 
  ggplot(.,aes(x = reorder(country,emissions_pc)))+
  geom_bar(aes(y = emissions_pc ,fill=region_ar6_10),stat='identity',colour="#525252")+
...
p5 <- countries %>% 
  ...
  #ggplot(.,aes(x = reorder(country,value),y = value*1000, fill=region_ar6_10)) +
  #geom_bar(stat='identity',colour="#737373") + 
  ggplot(.,aes(x = reorder(country,emissions_pgdp)))+
  geom_bar(aes(y = emissions_pgdp ,fill=region_ar6_10),stat='identity',colour="#525252")+
...
```{r regions_countries,echo=FALSE,warning=FALSE,fig.width=9,fig.height=12,dpi=300,fig.path="../../Results/Plots/",dev=c('png','pdf','svg')}
...
writeData(wb, sheet = "panel a", spread(plot_data,year,value), colNames = T, rowNames = F)
...
writeData(wb, sheet = "panel b-e",countries %>% filter(cutoff==1) %>% select(country,ISO,region_ar6_10,avg_growth,abs_growth,emissions_pc,emissions_pgdp), colNames = T, rowNames = F)

...
sectors <- edgar_raw %>%
  ...
  group_by(year,chapter_title,subsector_title,gas,gwp100_ar6) %>%
  summarise(value=sum(value,na.rm=TRUE)) %>%
  arrange(chapter_title,subsector_title,year)
...
trend_figure <- function(trend_data,chapter) {
  ...
  p <- trend_data %>%
    ...
    geom_text(data=shares %>% filter(subsector_title==shares$subsector_title[1]) %>% filter(year<=end_year),aes(x=ifelse(year==2010,year+4.5,year+5),y=max(shares$totals+ifelse(shares$subsector==1,5,1)),label=paste0(ifelse(total_rate>0,"+","-"),round(abs(total_rate),1),"%/yr")),size=3.5,colour="#252525") +

...
info <- data.frame(x=c("Author","Date","Units","Contact","Code"),y=c("William F. Lamb",as.character(Sys.Date()),"GtCO2eq","Lamb@mcc-berlin.net","https://github.com/mcc-apsis/AR6-Emissions-trends-and-drivers/blob/master/R/Analysis%20and%20figures/emissions_by_sector.Rmd"))
...
# AFOLU data for Jeremy

# load('../../Data/edgar6_v4_data_raw.RData')
# afolu <- edgar_raw %>% 
#   filter(chapter_title=="AFOLU") %>% 
#   group_by(year,chapter_title,subsector_title,gas,gwp100_ar6) %>% 
...
trend_figure <- function(trend_data,chapter) {
  ...
  p <- trend_data %>%
    ...
    geom_text(data=shares %>% filter(subsector_title==shares$subsector_title[1]),aes(x=year,y=1.05*max(shares$totals+ifelse(shares$subsector==1,5,1)),label=paste(signif(totals,2),  "Gt")),size=3.5,colour="#252525") +
    ...
    geom_text(data=shares %>% filter(subsector_title==shares$subsector_title[1]) %>% filter(year<=end_year),aes(x=ifelse(year==2010,year+4.5,year+5),y=max(shares$totals+ifelse(shares$subsector==1,5,1)),label=paste0(ifelse(total_rate>0,"+","-"),signif(abs(total_rate),2),"%")),size=3.5,colour="#252525") +
...
#   }
#   
#   label = data.frame(year=c(1990,2000,2010),rate_period=c("1990-2000","2000-2010",paste("2010-",end_year,sep="")))
#   
...
# 
# subsector_data <- gather(subsector_data,var,value,avg_growth_2010,abs_growth_2010)
# 
# 
# plot_theme <- theme_bw() +
...
#   scale_fill_brewer(type = "qual",palette="Set2") +
#   ylab("%") +
#   ggtitle(str_wrap("c. Avg. annual GHG emissions growth",width=38)) +
...
#   scale_fill_brewer(type = "qual",palette="Set2") +
#   ylab("Gt CO2eq") +
#   ggtitle(str_wrap("d. Absolute GHG emissions growth",width=35)) +
#     annotate(geom = 'text', label = 'Years: 2010-2019',
...
#sector_totals <- ggarrange(p1,p2,ggarrange(p3,p4,nrow = 1),nrow=3,align = "h")
sector_totals
...
openxlsx::addWorksheet(wb,paste0(chapters$chapter_title[i]," panel a"))
openxlsx::addWorksheet(wb,paste0(chapters$chapter_title[i]," panel b"))

openxlsx::writeData(wb, sheet = paste0(chapters$chapter_title[i]," panel a"),trend_data, colNames = T, rowNames = F)
openxlsx::writeData(wb, sheet = paste0(chapters$chapter_title[i]," panel b"),region_data, colNames = T, rowNames = F)

# openxlsx::addWorksheet(wb,"Emissions growth by subsector")
# openxlsx::writeData(wb, sheet = "Emissions growth by subsector", subsector_data, colNames = T, rowNames = F)
...
trend_data <- trend_data %>%
  ...
  mutate(subsector_title=ifelse(subsector_title=="Non-residential","Non-residential (direct CO2)",subsector_title)) %>%   mutate(subsector_title=ifelse(subsector_title=="Non-CO2 (all buildings)","All buildings (non-CO2)",subsector_title))
...
# Buildings (Scope 3) ->indirect CO2 only until 2018, compilation with end-year 2019 therefore failed

```{r buildings_with_scope3, echo=FALSE,dpi=300, fig.height=6, fig.path="../../Results/Plots/Sectors/", fig.width=8, warning=FALSE, dev=c('png','pdf','svg'), results='asis'}
...
p2 <- plot_data %>% 
  ...
  ggplot(.,aes(y="a",x=reorder(label,total_GHG_plus_indirect),label=paste0(ifelse(rate>0,"+","-"),signif(abs(rate)*100,2),"%"))) +
  geom_text() +
  coord_flip() +
  facet_grid(facet~.,scales="free",space="free") +
  theme(axis.title.y = element_blank(),
...
openxlsx::saveWorkbook(wb,"../../Results/Plot Data/ipcc_ar6_figure_sectors.xlsx",overwrite=T)

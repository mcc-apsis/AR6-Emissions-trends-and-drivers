...
wb <- openxlsx::createWorkbook(title = paste("ipcc_ar6_sector_trend_plots",Sys.Date()))
    ...
    if (ch!=6) {
      ...
      if (ch==9) {
        ...
        indirect_totals <- indirect_totals %>% 
          ...
          mutate(subsector_title=ifelse(subsector_title=="Non-residential","Indirect CO2 (non-residential)",subsector_title)) %>% 
          mutate(subsector_title=ifelse(subsector_title=="Residential","Indirect CO2 (residential)",subsector_title)) %>% 
          select(year,subsector,subsector_title,value)
      }
...
## align factors and labels
trend_data$subsector_title <- as.factor(trend_data$subsector_title)
trend_data$subsector_title <- factor(trend_data$subsector_title,levels=levels(trend_data$subsector_title)[c(2,1,5,4,3)])
...
## align factors and labels
region_data$subsector_title <- as.factor(region_data$subsector_title)
region_data$subsector_title <- factor(region_data$subsector_title,levels=levels(region_data$subsector_title)[c(2,1,5,4,3)])
...
```{r sector_ghgs,include=FALSE,echo=FALSE}
...
openxlsx::saveWorkbook(wb,paste0("../../Results/Data/ipcc_ar6_sector_trend_plots_",Sys.Date(),".xlsx"),overwrite=T)

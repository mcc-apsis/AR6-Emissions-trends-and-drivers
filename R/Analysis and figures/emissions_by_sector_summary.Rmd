...
subsector_data <- subsector_data %>%
  ...
  # mutate(avg_growth_1990=(last(value)/first(value))^(1/(last(year)-time_start))-1) %>%
  # mutate(abs_growth_1990=last(value)-first(value)) %>%
  filter(year>=2010 & year<=2019) %>%
  group_by(subsector,subsector_title) %>%
  mutate(avg_growth_2010=(last(value)/first(value))^(1/(last(year)-2010))-1) %>%
  mutate(abs_growth_2010=last(value)-first(value)) %>%
  filter(year==2019) %>%
...
p3 <- subsector_data %>%
  ...
  scale_fill_brewer(type = "qual",palette="Set2") +
  ylab("%") +
  ggtitle(str_wrap("c. Avg. annual GHG emissions growth by subsector",width=38)) +
...
p4 <- subsector_data %>%
  ...
  scale_fill_brewer(type = "qual",palette="Set2") +
  ylab("Gt CO2eq/yr") +
  ggtitle(str_wrap("d. Absolute GHG emissions growth by subsector",width=35)) +
    annotate(geom = 'text', label = 'Years: 2010-2019',
...
openxlsx::saveWorkbook(wb,paste0("../../Results/Plot data/ipcc_ar6_sector_summary.xlsx"),overwrite=T)

```



```{r sector_trends,echo=FALSE,warning=FALSE,results='asis',dpi=300,fig.width=10,fig.height=12,fig.path="../../Results/Plots/",dev=c('png','pdf','svg')}

...
```{r gather_data_geographic,include=FALSE}
...
```{r gather_data_sids_ldcs,include=FALSE}
...
data_ldc_sids <- rbind(co2_dev,co2_sids)
...
data_ldc_sids <- rbind(data_ldc_sids,pop_dev)
data_ldc_sids <- rbind(data_ldc_sids,pop_sids)
...
data_ldc_sids <- rbind(data_ldc_sids,co2_cons_dev)
...
ghg_sids <-  left_join(edgar_ghg,sids,by="ISO")
...
ghg_sids <- ghg_sids %>% 
  mutate(total= sum(ghg_sids$value)) %>% 
  mutate(fraction = value/total) %>% 
  mutate(label=paste0(round(total/1e9)," GtCO2eq")) %>% 
  filter(SIDS=="sids") %>% 
  rename(region=SIDS)
...
data_ldc_sids <- rbind(data_ldc_sids,ghg_dev)
data_ldc_sids <- rbind(data_ldc_sids,ghg_sids)


### CO2 1850-2019 excl LULUCF
...
co2_1850_sids <- left_join(hist_countries,sids,by = "ISO")

co2_1850_sids <- co2_1850_sids %>% 
  group_by(ISO,SIDS) %>% 
...
co2_1850_sids <- co2_1850_sids %>% 
  mutate(total=sum(co2_1850_sids$value)) %>% 
  mutate(fraction=value/total) %>% 
  mutate(label=paste0(round(total,0),"\nGtCO2eq")) %>% 
  mutate(var="Production CO2, excl. LULUCF (1850-2019)") %>% 
  mutate(category="b. Historical cumulative emissions by region") %>% 
...
data_ldc_sids <- rbind(data_ldc_sids,co2_1850_dev)
data_ldc_sids <- rbind(data_ldc_sids,co2_1850_sids)

data_ldc_sids <- data_ldc_sids %>% 
  add_row(var="Production GHG, incl. LULUCF (2019)",category="a. Current population and emissions by region",region="ldc") %>% 
  add_row(var="Production CO2, incl. LULUCF (1850-2019)",category="b. Historical cumulative emissions by region",region="ldc") %>% 
  add_row(var="Production CO2, incl. LULUCF (1850-2019)",category="b. Historical cumulative emissions by region",region="sids") %>% 
  add_row(var="Consumption CO2 FFI (2018)",category="a. Current population and emissions by region",region="sids") 

data_ldc_sids <- data_ldc_sids %>% 
  mutate(region=ifelse(region=="ldc","Least Developed Countries (LDCs)",region)) %>% 
  mutate(region=ifelse(region=="sids","Small Island Developing States (SIDS)",region))





data_ldc_sids$var <- as.factor(data_ldc_sids$var)
data_ldc_sids$category <- as.factor(data_ldc_sids$category)
data_ldc_sids$var <- fct_relevel(data_ldc_sids$var,"Population (2019)","Consumption CO2 FFI (2018)","Production CO2 FFI (2019)","Production GHG, excl. LULUCF (2019)","Production GHG, incl. LULUCF (2019)","Production CO2, excl. LULUCF (1850-2019)","Production CO2, incl. LULUCF (1850-2019)")
...
addWorksheet(wb,"geographic regions (top panels)")
addWorksheet(wb,"ldcs and sids (lower panels)")
...
writeData(wb, sheet = "geographic regions (top panels)", data, colNames = T, rowNames = F)
writeData(wb, sheet = "ldcs and sids (lower panels)", data_ldc_sids, colNames = T, rowNames = F)
...
```{r equity_perspectives,dpi=300,fig.width=11,fig.height=7,fig.path="../../Results/Plots/",dev=c('png','pdf')}
...
p1 <- data %>%
  ...
  geom_text(data=data,aes(x=var,y=position,label=paste0(round(fraction,2)*100,"%")),color="#525252") +
...
p2 <- data_ldc_sids %>% 
  ggplot(.,aes(x=var,y="a",label=ifelse(is.na(fraction),"",paste0(round(fraction*100,1),"%")))) +
  geom_text() +
  facet_grid(region~category,scales="free",space="free") +
...
p_labels_2 <- data_ldc_sids %>%
  filter(category=="b. Historical cumulative emissions by region") %>%
  filter(var=="Production GHG, incl. LULUCF (2019)") %>%
  ggplot(.,aes(x=1,y=c("a","b"),label=str_wrap(c("Small Island Developing States (SIDS)","Least Developed Countries (LDCs)"),width=25))) +
  geom_text(hjust=0,vjust=0.5,color="Grey",lineheight=0.75) +
  xlim(1,1.1) +
...
(p1 + p_labels + p2 + p_labels_2 + plot_layout(heights=c(7,1),ncol=2,widths=c(4,1)))

...
indirect_CO2_world <- gather(indirect_CO2_world,year,GHG,`1990`:`2018`)
...
sector_ghgs_regions <- sector_ghgs_regions %>% filter(year==2018)
...
# facet titles
titles <- data.frame(var=c("CO2","GHG","energy","energy_GDP","CO2_energy","GDP_POP","POP"),facet=c("CO2","GHG","Energy cons.","Energy intensity","Carbon intensity","GDP/ pop","Population"))

kaya_rates <- left_join(kaya_rates,titles,by = "var")

# reorder vars
kaya_rates$facet <- as.factor(kaya_rates$facet)
kaya_rates$facet <- factor(kaya_rates$facet,levels=levels(kaya_rates$facet)[c(6,2,1,4,3,5,7)])
...
kaya_rates_afolu <- gather(kaya_rates_afolu %>% 
                             select(-ag_production_kcal,-land_area_hectares,-GHG),var,value,population:ghg_land_area)
...
# facet titles
titles <- data.frame(var=c("population","GHG_total","production_percap","land_production","ghg_land_area"),facet=c("Population","GHG Total","Agricultural output","Land efficiency","GHG intensity"))

kaya_rates_afolu <- left_join(kaya_rates_afolu,titles,by = "var")

# reorder vars
kaya_rates_afolu$facet <- as.factor(kaya_rates_afolu$facet)
kaya_rates_afolu$facet <- factor(kaya_rates_afolu$facet,levels=levels(kaya_rates_afolu$facet)[c(3,2,4,1,5)])
...
trend_figure <- function(trend_data,sector) {
  ...
  p <- trend_data %>%
    ...
    geom_text(data=shares %>% filter(subsector_title==shares$subsector_title[1]),aes(x=year,y=1.09*max(shares$totals),label=paste(round(totals,1),  "Gt")),size=4,colour="#252525") +
...
fraction_figure <- function(sector_ghgs_regions,sector) {
  ...
  fraction_data <- sector_ghgs_regions %>% 
   ...
   title <- data.frame(title=ifelse(sector=="AFOLU","Emissions by region in 2018\n(all GHGs, incl. LULUCF CO2)","Emissions by region in 2018\n(all GHGs, incl. Indirect CO2)"))
  title <- title %>% 
    mutate(title=ifelse(sector=="Energy systems","Emissions by region in 2018\n(all GHGs)",title)) %>% 
    mutate(title=ifelse(sector=="Total","Emissions by region in 2018\n(all GHGs, incl. LULUCF CO2, excl. Indirect CO2",title))
...
kaya_figure <- function(kaya_rates,sector) {
  ...
  kaya_rates <- kaya_rates %>% 
   ...
   title <- data.frame(title=ifelse(sector=="AFOLU","c. Avg. annual change in kaya factors 2010-2018\n(GHG incl. LULUCF CO2)","c. Avg. annual change in kaya factors, 2010-2018\n(CO2 only, excl. indirect CO2)"))
  title <- title %>% 
    mutate(title=ifelse(sector=="Energy systems","c. Avg. annual change in kaya factors 2010-2018\n(CO2 only)",title)) %>% 
    mutate(title=ifelse(sector=="Total","c. Avg. annual change in kaya factors 2010-2018\n(CO2 only, excl. LULUCF CO2, excl. indirect CO2)",title))
  ...
  p <- kaya_rates %>% 
    ...
    facet_grid(.~facet,labeller = label_wrap_gen(width=12)) +
    theme(axis.title.y = element_blank(),
          axis.text.y = element_blank(),
          axis.ticks.y = element_blank(),
          axis.title.x = element_blank(),
...
```{r afolu_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
p1 <- trend_figure(trend_data,"AFOLU")
p2 <- fraction_figure(sector_ghgs_regions,"AFOLU")
p2 <- p2 + ylim(0,40)
p3 <- per_capita_figure(per_capita,per_capita_world,sector_ghgs_regions,"AFOLU")
p3 <- p3 + ylim(0,6)
p4 <- kaya_figure(kaya_rates_afolu,"AFOLU")

(wrap_elements(p1) / wrap_elements(p2 + p3 + p4 + plot_layout(widths=c(1,1.5,3)))) + plot_layout(heights=c(1,1.25))
...
```{r energy_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
trend_data$subsector_title <- as.factor(trend_data$subsector_title)
trend_data$subsector_title <- factor(trend_data$subsector_title,levels=levels(trend_data$subsector_title)[c(3,4,5,2,6,1)])

p1 <- trend_figure(trend_data,"Energy systems")
p2 <- fraction_figure(sector_ghgs_regions,"Energy systems")
p3 <- per_capita_figure(per_capita,per_capita_world,sector_ghgs_regions,"Energy systems")
p3 <- p3 + ylim(0,25)
p4 <- kaya_figure(kaya_rates,"Energy systems")
...
```{r industry_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
p1 <- trend_figure(trend_data,"Industry")
p2 <- fraction_figure(sector_ghgs_regions,"Industry")
p2 <- p2 + ylim(0,60)
p3 <- per_capita_figure(per_capita,per_capita_world,sector_ghgs_regions,"Industry")
p4 <- kaya_figure(kaya_rates,"Industry")
...
```{r buildings_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
p1 <- trend_figure(trend_data,"Buildings")
p2 <- fraction_figure(sector_ghgs_regions,"Buildings")
p2 <- p2 + ylim(0,40)
p3 <- per_capita_figure(per_capita,per_capita_world,sector_ghgs_regions,"Buildings")
p4 <- kaya_figure(kaya_rates,"Buildings")
...
```{r transport_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
p1 <- trend_figure(trend_data,"Transport")
p2 <- fraction_figure(sector_ghgs_regions,"Transport")
p2 <- p2 + ylim(0,40)
p3 <- per_capita_figure(per_capita,per_capita_world,sector_ghgs_regions,"Transport")
p3 <- p3 + ylim(0,7.5)
p4 <- kaya_figure(kaya_rates,"Transport")
...
```{r total_summary,echo=FALSE,warning=FALSE,fig.width=10,fig.height=8,fig.path="../../Results/Plots/Sectors/",dev=c('png','pdf')}
...
p1 <- trend_figure(overall_ghg_totals,"Total")
p2 <- fraction_figure(overall_ghg_regions,"Total")
p2 <- p2 + ylim(0,40)
p3 <- per_capita_figure(per_capita %>% mutate(chapter_title="Total"),per_capita_world,overall_ghg_regions,"Total")
#p3 <- p3 + ylim(0,7.5)
p4 <- kaya_figure(kaya_rates,"Total")

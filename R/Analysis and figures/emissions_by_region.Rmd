...
region_rates <- region_rates %>% 
  ...
  group_by(country,region_ar6_6) %>% 
  summarise(rate=(last(emissions_total)/first(emissions_total))^(1/(last(year)-first(year)))-1) %>% 
  ungroup() %>% 
  mutate(rate=rate*100) %>% 
  mutate(aggregated_category=region_ar6_6) %>% 
  select(country,aggregated_category,rate) %>% 
  mutate(facet="Average annual emissions change (2010-2018)")
...
```{r jitter_plot_version, echo=FALSE,warning=FALSE,fig.width=8,fig.height=3}
...
p2 <- region_rates %>% 
  ggplot(.,aes(x=aggregated_category,rate,fill=aggregated_category,color=aggregated_category)) +
  geom_hline(yintercept=0,color="#636363") +
...
addWorksheet(wb,"panel_b_summary")
...
writeData(wb, sheet = "panel_b_summary", region_rates_bars %>% select(-id), colNames = T, rowNames = F)

openxlsx::saveWorkbook(wb,paste0("../../Results/Data/ipcc_ar6_figure_spm2_regions.xlsx"),overwrite=T)

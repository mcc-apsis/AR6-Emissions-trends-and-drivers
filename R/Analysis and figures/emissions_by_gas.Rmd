...
load('../../Data/edgar6_data_raw_gwp_ar6.RData')

#load('../../Data/edgar6_data_gwp_ar5.RData')
#load('../../Data/edgar6_data_all_gwp_ar5.RData')
...
wb <- openxlsx::createWorkbook(title = paste("ipcc_ar6_gas_data"))
...
methane <- edgar_GHG %>% 
  filter(gas=="CH4") %>% 
  group_by(sector_code,fossil_bio,description,year) %>% 
  summarise(value=sum(value,na.rm=TRUE))

methane <- left_join(methane,gwps_ch4 %>% select(sector_code,fossil_bio,gwp_ar6),by = c("sector_code","fossil_bio"))

any_missing <- anti_join(methane,gwps_ch4 %>% select(sector_code,fossil_bio,gwp_ar6),by = c("sector_code","fossil_bio"))

methane <- methane %>% 
  mutate(value=value*gwp_ar6) %>% 
  group_by(year) %>% 
  summarise(gwp_ar6_ch4=sum(value,na.rm=TRUE)/1e9) %>% 
  mutate(gas="CH4")

waterfall_data <- left_join(waterfall_data,methane,by=c("gas","year"))
waterfall_data <- waterfall_data %>% 
  mutate(gwp_ar6=ifelse(gas=="CH4",gwp_ar6_ch4,gwp_ar6)) %>% 
  select(-gwp_ar6_ch4)

# methane <- methane %>% 
#   mutate(CH4_adjusted=CH4*gwp_ar6) %>% 
...
p_waterfall <- waterfall_data %>%
  ...
  geom_text(data=waterfall_data %>% filter(gwp!="gwp_ar4") %>%  filter(gas=="CO2 Land use"),aes(x=gas,y=62,label=paste(gwp,"\n",round(total,1),  "Gt")),size=3.5,colour="#252525")+
  
  theme_bw() +
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60),limits=c(0,62)) +
  scale_x_discrete(labels=c("","","","","")) +
  #xlab("GWP AR2, AR4, AR5, AR6") +
  theme(
...
land_totals <- land %>% 
  ...
  filter(year<2020) %>% 
  group_by(year) %>% 
  summarise(value=sum(mean)/1e9) %>% 
  mutate(gas="CO2 Land use")

plot_data_nogwp <- rbind(plot_data_nogwp,land_totals)
...
plot_data_nogwp <- left_join(plot_data_nogwp,fgas_list,by = "gas")

fgases <- left_join(plot_data_nogwp %>% filter(fgas==1),gwps %>% select(gas,gwp_ar6),by="gas")
...
plot_data_nogwp <- rbind(plot_data_nogwp %>% filter(is.na(fgas)) %>% select(-fgas),fgases)


# Normalise to 1 in first year?
plot_data_nogwp <- plot_data_nogwp %>%
  filter(year>1989) %>% 
  group_by(gas) %>%
...
plot_data_nogwp$gas <-  factor(plot_data_nogwp$gas,levels(plot_data_nogwp$gas)[c(2,3,1,5,4)])
...
openxlsx::saveWorkbook(wb,paste0("../../Results/Data/ipcc_ar6_gas_data.xlsx"),overwrite=T)

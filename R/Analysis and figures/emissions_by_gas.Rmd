...
spm_table <- spm_table %>% 
  filter(year==2019) %>% 
  group_by(gas) %>% 
  summarise(level=paste0(signif(level,2),"±",signif(uncertainty,2)))
...
growth <- left_join(growth,uncertainties,by = "gas")
growth <- growth %>% 
  mutate(uncertainty=uncertainty*growth_abs) %>% 
  mutate(uncertainty2=uncertainty^2)
growth <- growth %>% 
  mutate(uncertainty=ifelse(gas=="Total",sqrt(sum(growth$uncertainty2,na.rm=TRUE)),uncertainty)) %>% 
  select(-uncertainty2)

growth <- growth %>%
  mutate(growth_abs=paste0(signif(growth_abs,2),"±",signif(uncertainty,2))) %>% 
  mutate(growth_rel=signif(growth_rel,2)) %>% 
  select(-uncertainty)

spm_table <- left_join(spm_table,growth,by = "gas")
names(spm_table) <- c("gas","2019 Emissions (GtCO2eq)","1990-2019 Growth (GtCO2eq)","1990-2019 Growth (%)") 
...
writeData(wb, sheet = "panel a", table <- spread(table,year,value) %>% arrange(var), colNames = T, rowNames = F)
writeData(wb, sheet = "panel a growth rates", rates, colNames = T, rowNames = F)
writeData(wb, sheet = "panel a shares", shares, colNames = T, rowNames = F)
writeData(wb, sheet = "panel b", waterfall_data, colNames = T, rowNames = F)
writeData(wb, sheet = "panel c", gas_trend_nogwp, colNames = T, rowNames = F)
writeData(wb, sheet = "spm table", spm_table, colNames = T, rowNames = T)

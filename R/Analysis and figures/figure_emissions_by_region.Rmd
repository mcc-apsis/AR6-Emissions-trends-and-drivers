...
region_trend <- region_trend %>%
  mutate(region_ar6_10=ifelse(region_ar6_6_short=="AIR","Intl. Shipping & Aviation",region_ar6_10)) %>% 
  mutate(region_ar6_10=ifelse(region_ar6_6_short=="SEA","Intl. Shipping & Aviation",region_ar6_10))
...
region_trend <- region_trend %>% 
  ...
  mutate(value=ifelse(region_ar6_10!="Intl. Shipping & Aviation",value+mean,value))


## merge aviation and shipping
...
## adjust levels, as in SPM2
region_trend$region_ar6_10 <- as.factor(region_trend$region_ar6_10)
region_trend$region_ar6_10 <- fct_relevel(region_trend$region_ar6_10,
...
colors = c("#B3B3B3","#66C2A5","#CF9C76","#CF948C","#969DCA","#D58EC4","#C6B18B","#B7D84C","#F6D832","#EFCC6B","#D5BE9D")
...
shares <- locate_shares(shares,"region_ar6_10",4)
...
p1 <- region_trend %>%
  ...
  geom_text(data=shares %>% filter(region_ar6_10=="Africa"),aes(x=year,y=62,label=paste(signif(totals,2),  "Gt")),size=3.5,colour="#252525")+
  ...
  coord_cartesian(xlim = c(1990,2020.5)) +
...
#   ggplot(.,aes(x=year,y=value,fill=region_ar6_10)) +
#   geom_text(data=shares %>% filter(year==2019),aes(x=1,y=location,label=str_wrap(region_ar6_10,width=28),color=region_ar6_10,lineheight=0.65),hjust=0) +
...
# Note, this data must be downloaded from the IIASA AR6 Integratted Assessment Scenario repository 

scenario_data <- read.xlsx("../../Data/Not public/supplemetary data/ar6_full_metadata_indicators2021_10_14_v7.xlsx",
                          sheet=4)
...
region_rates <- edgar_ghg %>% 
  ...
  group_by(country,ISO,year,region_ar6_10) %>% 
...
region_rates <- region_rates %>% 
  ...
  group_by(country,ISO,region_ar6_10) %>% 
  ...
  mutate(aggregated_category=region_ar6_10) %>% 
...
# join data
region_rates <- left_join(region_rates,region_averages_nolulucf,by=c("aggregated_category"="region_ar6_10"))
region_rates <- region_rates %>% 
...
## rename scenarios for consistency with SPM
region_rates <- region_rates %>% 
  mutate(aggregated_category=ifelse(aggregated_category=="C3a: Likely below 2°C with immediate action",
                                    "C3a: Likely below 2°C with action starting in 2020",aggregated_category)) %>%   mutate(aggregated_category=ifelse(aggregated_category=="C1: Below 1.5°C with no or low overshoot",
                                    "C1: 1.5°C with no or limited overshoot",aggregated_category))


## adjust levels, as in SPM2
region_rates$aggregated_category <- as.factor(region_rates$aggregated_category)
region_rates$aggregated_category <- fct_relevel(region_rates$aggregated_category,
                                      "Australia, Japan, New Zealand",
                                      "Middle East",
                                      "Eastern Europe and West-Central Asia",
                                      "Europe",
                                      ...
                                      "North America",
                                      "Eastern Asia",
                                      "C3a: Likely below 2°C with action starting in 2020",
                                      "C1: 1.5°C with no or limited overshoot")
...
#                                                 "C1: Below 1.5°C with no or low overshoot",
#                                                 after=Inf)
...
```{r regions_scenarios, echo=FALSE,warning=FALSE,fig.width=9,fig.height=3,dpi=300,fig.path="../../Results/Plots/",dev=c('png','pdf','svg')}

colors = c("#66C2A5","#CF9C76","#CF948C","#969DCA","#D58EC4","#C6B18B","#B7D84C","#F6D832","#EFCC6B","#D5BE9D","#B3B3B3","#B3B3B3")


p2 <- region_rates %>% 
    mutate(facet=ifelse(facet=="Scenario reduction rates (2020-2040)","Scenario reduction\n rates (2020-2040)",facet)) %>% 
  ...
  stat_summary(data = region_summary %>% mutate(facet=ifelse(facet=="Scenario reduction rates (2020-2040)","Scenario reduction\n rates (2020-2040)",facet)), geom = "crossbar",
...
```{r region_trends_spm, echo=FALSE,warning=FALSE,fig.width=9,fig.height=8,dpi=300,fig.path="../../Results/Plots/",dev=c('png','pdf','svg')}

p2 <- p2 + ggtitle("b. Recent emission change by region vs. global reduction rates compatible with warming targets")
regions <- wrap_elements(p1) / wrap_elements(p2) + plot_layout(heights = c(1.5, 1))
regions
...
writeData(wb, sheet = "panel a", spread(region_trend,year,value), colNames = T, rowNames = F)

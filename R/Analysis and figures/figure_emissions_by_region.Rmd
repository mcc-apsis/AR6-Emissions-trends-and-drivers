...
region_trend$region_ar6_10 <- fct_relevel(region_trend$region_ar6_10,"Intl. Shipping & Aviation",after=Inf)
...
fraction <- fraction %>% 
  ...
  mutate(threshold_CO2=ifelse(CO2_pc>12,">12tCO2/cap",NA)) %>% 
  mutate(threshold_CO2=ifelse(CO2_pc<3,"<3tCO2/cap",threshold_CO2)) %>% 
  mutate(GHG_pc=GHG/population) %>% 
  mutate(threshold_GHG=ifelse(GHG_pc>12,">12tCO2/cap",NA)) %>% 
  mutate(threshold_GHG=ifelse(GHG_pc<3,"<3tCO2/cap",threshold_GHG)) 

summary_CO2 <- fraction %>% 
  group_by(threshold_CO2) %>% 
  summarise(population=sum(population,na.rm=TRUE)/sum(fraction$population,na.rm=TRUE),emissions=sum(CO2)/sum(fraction$CO2))

summary_regions_CO2 <- fraction %>% 
  filter(threshold_CO2=="<3tCO2/cap")
summary_regions_CO2 <- summary_regions_CO2 %>% 
  group_by(region_ar6_10) %>% 
  summarise(population=sum(population,na.rm=TRUE)/sum(summary_regions_CO2$population,na.rm=TRUE))

summary_GHG <- fraction %>% 
  group_by(threshold_GHG) %>% 
  summarise(population=sum(population,na.rm=TRUE)/sum(fraction$population,na.rm=TRUE),emissions=sum(GHG)/sum(fraction$GHG))

summary_regions_GHG <- fraction %>% 
  filter(threshold_GHG=="<3tCO2/cap")
summary_regions_GHG <- summary_regions_GHG %>% 
  group_by(region_ar6_10) %>% 
  summarise(population=sum(population,na.rm=TRUE)/sum(summary_regions_GHG$population,na.rm=TRUE))
...
addWorksheet(wb2,"summary_CO2")
addWorksheet(wb2,"summary_GHG")
addWorksheet(wb2,"summary_CO2_low_emissions")
addWorksheet(wb2,"summary_GHG_low_emissions")


writeData(wb2, sheet="data",fraction,colNames=T,rowNames=F)
writeData(wb2, sheet="summary_CO2",summary_CO2,colNames=T,rowNames=F)
writeData(wb2, sheet="summary_GHG",summary_GHG,colNames=T,rowNames=F)
writeData(wb2, sheet="summary_CO2_low_emissions",summary_regions_CO2,colNames=T,rowNames=F)
writeData(wb2, sheet="summary_GHG_low_emissions",summary_regions_GHG,colNames=T,rowNames=F)

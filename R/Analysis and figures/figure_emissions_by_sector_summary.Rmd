...
land <- land %>%
  ...
  select(region_ar6_10,region_ar6_10_short,year,sector_title,subsector_title,value=mean)
...
trend_data$sector_title <- as.factor(trend_data$sector_title)
trend_data$sector_title <- factor(trend_data$sector_title,levels=levels(trend_data$sector_title)[c(3,4,1,5,2)])
...
shares <- locate_shares(shares,"sector_title",4)
...
p1 <- trend_data %>%
  ggplot(.,aes(x=year,y=value,fill=sector_title)) +
  ...
  geom_text(data=shares %>% filter(sector_title==shares$sector_title[1]),aes(x=year,y=62,label=paste(round(totals,0),  "Gt")),size=3.5,colour="#252525")+
...
region_data <- edgar_ghg %>% 
  ...
  filter(year %in% c(1990,2000,2010,end_year)) %>% 
  group_by(sector_title,region_ar6_10,region_ar6_10_short,year) %>% 
...
land_regions <- land %>% 
  filter(year %in% c(1990,2000,2010,end_year)) %>% 
  select(sector_title,region_ar6_10,region_ar6_10_short,year,value) %>% 
  mutate(value=value/1e9)
...
## align factors and labels
region_data$sector_title <- as.factor(region_data$sector_title)
region_data$sector_title <- factor(region_data$sector_title,levels=levels(region_data$sector_title)[c(3,4,1,5,2)])

colours <- c("#66c2a5","#fc8d62","#8da0cb","#e78ac3","#a6d854","#ffd92f","#e5c494")


p2 <- region_data %>% ggplot(.,aes(x=year,y=value,fill=sector_title)) +
...
## align factors and labels
subsector_data$sector_title <- as.factor(subsector_data$sector_title)
subsector_data$sector_title <- factor(subsector_data$sector_title,levels=levels(subsector_data$sector_title)[c(3,4,1,5,2)])
...
p3 <- subsector_data %>%
  ...
  ggplot(.,aes(x = reorder(subsector_title,value),y = value, fill=sector_title)) +
  geom_bar(stat='identity',colour="#737373") +
  coord_flip() +
  plot_theme +
  scale_fill_brewer(type = "qual",palette="Set2") +
...
openxlsx::writeData(wb, sheet = "emissions by sector and region", spread(region_data %>% ungroup(),sector_title,value), colNames = T, rowNames = F)
openxlsx::writeData(wb, sheet = "emissions by subsector", subsector_data, colNames = T, rowNames = F)


openxlsx::saveWorkbook(wb,paste0("../../Results/Plot data/ipcc_ar6_figure_sector_summary_1990.xlsx"),overwrite=T)

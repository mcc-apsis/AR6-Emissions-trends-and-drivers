---
title: "Global emissions trends by region"
...
plot_data <- left_join(plot_data,land %>% select(-blue,-houghton),by=c("year","region_ar6_10"))
plot_data <- plot_data %>% 
...
p1 <- plot_data %>%
  ...
  #### labels
  geom_text(data=shares %>% filter(year==2018),aes(x=2020,y=location,label=region_ar6_5,color=region_ar6_5_short),hjust=0) +
  
  
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60)) +
  scale_x_continuous(breaks=c(1990,2000,2010,2018),limits = c(1990,2032)) +
  big_trend_theme +
...
```{r benchmark_data, echo=FALSE,warning=FALSE,fig.width=8,fig.height=4}
...
scenario_data <- scenario_data %>% 
  ...
  mutate(aggregated_category=ifelse(aggregated_category=="High 2C","2oC warming (66% probability)",aggregated_category)) %>% 
  mutate(aggregated_category=ifelse(aggregated_category=="Low 2C","2oC warming (50% probability)",aggregated_category))
...
region_rates <- region_rates %>% 
  ...
  select(aggregated_category,rate) %>% 
  mutate(facet="Historical reduction \nrates (2010-2018)")
  
#blarg <- rbind(blarg,benchmark_data %>% mutate(facet="Scenarios (2020-2050)"))
region_rates <- rbind(region_rates,scenario_data %>% mutate(facet="Scenario reduction \nrates (2020-2040)"))

region_rates <- region_rates %>% 
  mutate(aggregated_category = as.factor(aggregated_category))

region_rates$aggregated_category <- factor(region_rates$aggregated_category,levels= levels(region_rates$aggregated_category)[c(4,5,6,7,8,1,2,3)])
...
```{r spm_region_trends, echo=FALSE,warning=FALSE,fig.width=8,fig.height=7,fig.path="../../Results/Plots/",dev=c('png','pdf')}

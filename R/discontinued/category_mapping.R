...
ipcc_ar5_categories <- openxlsx::read.xlsx('Data/Codes and classifications/IPCC_AR5_sector_mapping.xlsx',sheet='EmissionMap',startRow = 3) %>% 
  select(code=IPCC.cat.,IPCC_AR5_sector=Sector_A,IPCC_AR5_chapter=Chapter_a,IPCC_AR5_description=IPCC_description,subsector=Sector_b) %>% 
...
master_list <- master_list %>% 
  ...
  arrange(code) %>% 
  select(code,IPCC_AR6_chapter,IPCC_AR6_chapter_title,IPCC_AR2_description,IPCC_AR5_description,EDGAR_description,combined_category=category,source,subsector)
...
ipcc_categories <- left_join(ipcc_categories,master_list %>% select(code,IPCC_AR6_chapter,IPCC_AR6_chapter_title,subsector),by=("code"="code")) %>%
...
ipcc_categories <- left_join(ipcc_categories,edgar_categories %>% mutate(keep=1) %>% select(code,keep),by=c("code"="code"))
ipcc_categories <- ipcc_categories %>% 
  filter(keep==1) %>% 
  select(-keep)
...
############ EDGAR codes with the same description

library(janitor)
blarg <- edgar_categories %>% 
  get_dupes(EDGAR_description)

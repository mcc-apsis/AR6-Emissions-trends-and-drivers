...
```{r total_sector_decomp,echo=FALSE,warning=FALSE,fig.width=8,fig.height=10,fig.path="../Results/Plots/",dev=c('png','pdf')}
...
trend <- trend$plot
trend <- trend + ggtitle("A. Trend in total emissions and income classification ")
d1 <- trend$data
...
load('../Data/edgar.RData')
  ...
  ### join and plot
  rates <- gather(blarg %>% mutate(rate=rate*100),var,value,rate:abs_growth)
  rates$chapter <- as.factor(rates$chapter)
  

  #rate
  p1 <- rates %>% 
    filter(var=="rate") %>% 
    ggplot(.,aes(x = reorder(chapter,value),y = value,fill=chapter)) +
    geom_bar(stat='identity') + 
  ...
  p2 <- rates %>% 
    ...
    ggplot(.,aes(x = reorder(chapter,value),y = value/1e6, fill=chapter)) +
    geom_bar(stat='identity') + 
...
plot <- annotate_figure(plot,
  top = text_grob("B. Decomposition of relative and absolute contribution by sector",size = 12,hjust=0.75))

plot <- ggarrange(trend,plot,ncol=1,nrow=2,heights=c(0.6,0.4))
plot

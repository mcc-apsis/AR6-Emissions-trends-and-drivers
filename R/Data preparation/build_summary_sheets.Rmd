...
regions_ar6 <- left_join(regions_ar6,land %>% 
                           filter(year==2019) %>% 
                           select(region_ar6_10,CO2_LULUCF=mean) %>% 
                           mutate(chapter_title="AFOLU"),by=c("region_ar6_10","chapter_title"))


regions_ar6 <- gather(regions_ar6,gas,value,CO2:CO2_LULUCF) %>% 
  mutate(value=value/1e9)
regions_ar6$gas <- as.factor(regions_ar6$gas)
regions_ar6$gas <- fct_relevel(regions_ar6$gas,"CO2","CH4","N2O","Fgas","CO2_LULUCF")
...
only_ipcc_sectors <- sectors_gases %>% 
  ungroup() %>% 
...
only_edgar_sectors <- sectors_gases %>% 
  ungroup() %>% 
  select(-gas,-ipcc_2006) %>% 
  distinct() %>% 
  group_by(chapter_title,subsector_title) %>% 
  summarise(edgar_code=paste0(sector_code,collapse=", "))
...
sectors_gases <- left_join(only_edgar_sectors,only_ipcc_sectors,by = c("chapter_title", "subsector_title"))
sectors_gases <- left_join(sectors_gases,only_gases,by = c("chapter_title", "subsector_title"))


addWorksheet(wb2,"sector_table_small")
writeData(wb2, sheet = "sector_table_small",sectors_gases, colNames = T, rowNames = F)
...
annex <- edgar_raw %>%
  group_by(chapter_title,subsector_title,sector_code,fossil_bio,description,gas) %>%
  summarise(value=sum(value,na.rm=TRUE))

annex <- annex %>%
  group_by(chapter_title,subsector_title,sector_code,fossil_bio,description) %>%
  summarise(gases = paste0(gas,collapse=", "))
...
annex_gases <- annex %>% select(-sector_code,-ipcc_2006)
annex_codes <- annex %>% 
  group_by(chapter_title,subsector_title,description) %>%
  summarise(edgar_code=paste0(sector_code,collapse=", "))
annex_ipcc <- annex %>% 
  group_by(chapter_title,subsector_title,description) %>%
  summarise(ipcc_2006=paste0(ipcc_2006,collapse=", "))

annex <- left_join(annex_gases,annex_codes,by = c("chapter_title", "subsector_title", "description"))
annex <- left_join(annex,annex_ipcc,by = c("chapter_title", "subsector_title", "description"))

annex <- annex %>% 
  mutate(ipcc_2006=ifelse(ipcc_2006=="NA (fossil)",NA,ipcc_2006))%>% 
  mutate(ipcc_2006=ifelse(ipcc_2006=="NA (bio)",NA,ipcc_2006)) %>% 
  select(chapter_title,subsector_title,description,edgar_code,ipcc_2006,gases)

addWorksheet(wb2,"sector_table_large")
writeData(wb2, sheet = "sector_table_large",annex, colNames = T, rowNames = F)

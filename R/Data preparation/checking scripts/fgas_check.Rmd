...
fgas_data_v6_new <- edgar_raw_v6_new %>% 
  group_by(year,gas,gwp100_ar5,gwp100_ar6) %>% 
  summarise(value=sum(value,na.rm=TRUE)) %>% 
  mutate(version="edgar_v6_new")

fgas_data <- rbind(fgas_data_v5,fgas_data_v6_new)
...
fgas_data_v6_new_gwp <- edgar_ghg_v6_new %>% 
  group_by(year) %>% 
  summarise(value=sum(Fgas,na.rm=TRUE)/1e9) %>% 
  mutate(version="edgar_v6_new_ar6_gwp")

fgas_data_gwp <- rbind(fgas_data_v5_gwp,fgas_data_v6_new_gwp)
...
fgas_data_v6_old <- edgar_raw_v6_old %>%  
  group_by(year,gas,gwp100_ar5,gwp100_ar6) %>% 
  summarise(value=sum(value,na.rm=TRUE)) %>% 
  mutate(version="edgar_v6_old")
...
fgas_data <- rbind(fgas_data_v6_old,fgas_data_v6_new)
...
fgas_data_v6_old_gwp <- edgar_ghg_v6_old %>%  
  group_by(year) %>% 
  summarise(value=sum(Fgas,na.rm=TRUE)/1e9) %>% 
  mutate(version="edgar_v6_old_ar6_gwp")
...
fgas_data_gwp <- rbind(fgas_data_v6_old_gwp,fgas_data_v6_new_gwp)
...
load('../../../Data/edgar6_data_ghg_gwp_ar5_0721.RData')
edgar_ghg_v6_new_ar5 <- edgar_ghg %>%
  filter(!is.na(Fgas))

# fgas_data_v5_ar5_gwp <- edgar_ghg_v5 %>%  
#   group_by(year) %>% 
#   summarise(value=sum(Fgas,na.rm=TRUE)/1e9) %>% 
#   mutate(version="edgar_v5_ar5_gwp")
edgar_ghg_v6_new_ar5<-left_join(edgar_ghg_v6_new_ar5,annexIcountries,by=c("ISO"="countries")) %>%
  filter(!is.na(annexI))

fgas_data_v6_new_gwp_ar5 <- edgar_ghg_v6_new_ar5 %>%
  group_by(year) %>%
  summarise(value=sum(Fgas,na.rm=TRUE)/1e9) %>%
  mutate(version="edgar_v6_new_ar5_gwp_annexI")

fgas_data_gwp <- rbind(fgas_data_v6_new_gwp_ar5,crf_fgas)
...
#   mutate(edgar_v6_new_gwp_ar5 = edgar_v6_new*gwp100_ar5) %>% 
#   mutate(edgar_v5_gwp_ar6 = edgar_v5*gwp100_ar6) %>% 
#   mutate(edgar_v6_new_gwp_ar6 = edgar_v6_new*gwp100_ar6)
# 

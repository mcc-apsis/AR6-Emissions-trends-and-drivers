...
edgar_raw_v1 <- edgar_raw %>% mutate(version="edgar6_v4")

load('../../../Data/edgar6_v5_data_raw.RData')
edgar_raw_v2 <- edgar_raw %>% mutate(version="edgar6_v5")
rm(edgar_raw)
...
edgar_ghg_v1 <- edgar_ghg %>% mutate(version="edgar6_v4")

load('../../../Data/edgar6_v5_data_ghg_gwp_ar6.RData')
edgar_ghg_v2 <- edgar_ghg %>% mutate(version="edgar6_v5")
rm(edgar_ghg)



load('../../../Data/WDI_gdp_pop.RData')
edgar_ghg_v1 <- left_join(edgar_ghg_v1,wdi_data_gdp_pop %>% select(ISO=iso3c,year,population), by = c("ISO", "year"))
edgar_ghg_v2 <- left_join(edgar_ghg_v2,wdi_data_gdp_pop %>% select(ISO=iso3c,year,population), by = c("ISO", "year"))
...
plot_data_v1 <- edgar_raw_v1 %>%  
  group_by(year,gas,version) %>% 
  summarise(value=sum(value,na.rm=TRUE))

plot_data_v2 <- edgar_raw_v2 %>% 
  group_by(year,gas,version) %>% 
  summarise(value=sum(value,na.rm=TRUE))

plot_data <- rbind(plot_data_v1,plot_data_v2)
...
```{r global_trend_fgas_gwps,fig.width=4,fig.height=4}
...
sector_total_v2 <- edgar_ghg_v2 %>% 
  group_by(year,sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
  summarise_at(vars(CO2,CH4,N2O,Fgas,GHG),sum,na.rm=TRUE) %>% 
  mutate(label=paste0(sector_code,"_",fossil_bio))

sector_total_v2 <- gather(sector_total_v2 %>% select(-GHG),gas,value,CO2:Fgas) %>% 
  arrange(chapter_title) %>% 
  mutate(value=value/1e9)

sector_total_v1 <- edgar_ghg_v1 %>% 
  group_by(year,sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
...
sector_countries_v2 <- edgar_ghg_v2 %>% 
  filter(year==2018) %>% 
  group_by(ISO,country,sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
  summarise(value_pc=GHG/population)

sector_global_v2  <- edgar_ghg_v2 %>% 
  filter(year==2018) %>% 
  group_by(sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
  summarise(value_pc=sum(GHG,na.rm=TRUE)/sum(population,na.rm=TRUE)) %>% 
  mutate(ISO="WLD") %>% 
  mutate(country="World")

sector_countries_v2 <- rbind(sector_countries_v2,sector_countries_v2)

sector_countries_v1 <- edgar_ghg_v1 %>% 
  filter(year==2018) %>% 
  group_by(ISO,country,sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
  summarise(value_pc=GHG/population)

sector_global_v1 <- edgar_ghg_v1 %>% 
  filter(year==2018) %>% 
  group_by(sector_code,fossil_bio,description,chapter_title,subsector_title,version) %>% 
  summarise(value_pc=sum(GHG,na.rm=TRUE)/sum(population,na.rm=TRUE)) %>% 
  mutate(ISO="WLD") %>% 
...
sector_countries_v1 <- rbind(sector_countries_v1,sector_global_v1)

sector_countries <- rbind(sector_countries_v1,sector_countries_v2) %>% 
  mutate(label=paste0(sector_code,"_",fossil_bio))

for (i in 1:length(unique(sector_total_v2$chapter_title))) {

  chapter_handle = unique(sector_total_v2$chapter_title)[i]
  cat("\n\n### ", chapter_handle,"sectors" , "\n")
  
  data <- sector_total_v2 %>% 
    filter(chapter_title==chapter_handle)
  data_v5 <- sector_total_v1 %>% 
    filter(chapter_title==chapter_handle)
...
plot_data_gases_v1 <- edgar_ghg_v1 %>% 
  group_by(year,region_ar6_10,version) %>% 
  summarise(value=sum(GHG,na.rm=TRUE)/1e9)

plot_data_gases_v1 <- left_join(plot_data_gases_v1,numbers,by="region_ar6_10")
...
for (i in c(1,6)) {
  ...
  p1 <- plot_data_gases %>% 
    ...
    ggplot(.,aes(x=year,y=value,fill=gas)) +
    geom_area(colour="#737373") +
    geom_path(data=plot_data_gases_v1 %>% filter(number>=i) %>% filter(number<i+6), aes.inherit=FALSE,aes(x=year,y=value,fill=NA,color=version),size=0.5) +
    scale_color_manual(values="#b30000") +
    ylab("GHG emissions (GtCO2eq)") +
...
plot_data_subsectors <- edgar_ghg_v2 %>% 
...
plot_data_subsectors_v1 <- edgar_ghg_v1 %>% 
  group_by(year,country,chapter_title,subsector_title,version) %>% 
  summarise(value=sum(GHG,na.rm=TRUE)/1e6) 

# country list: take the top 150

country_list <- edgar_ghg_v1 %>% 
  filter(year==2019) %>% 
  group_by(country) %>% 
...
#for (i in 41:42) {
  ...
  country_handle = country_list$country[i]
  ...
  if (length(plot_data_gases_v1$value[plot_data_gases_v1$country==country_handle])>0) {
    
    p1 <- p1 +
      geom_path(data=plot_data_gases_v1 %>% filter(country==country_handle), inherit.aes=FALSE,aes(x=year,y=value,fill=NA,color=version),size=0.5) +
    scale_color_manual(values="#b30000")
  ...
  data = plot_data_subsectors_v1 %>% filter(country==country_handle) %>% filter(year==2018)

...
## this script takes the IEA CO2 emissions data

rm(list = ls())
library(tidyverse)
library(openxlsx)

# we must use the non public EDGAR data in order to precisely match the sector codes
...
iea <- read.csv(file="../../Data/Not public/IEA/2021 update/iea_scope2_2021.csv",sep = ";")
...
### calculate indirect CO2 for each sector

iea <- spread(iea,var,value)
iea <- iea %>% 
  mutate(indirect_CO2 = CO2_plus_elec_heat-CO2)

### remove aggregate categories in order to avoid double counting
# industry and transport totals, manufacturing sector (a new aggregate in the 2021 version)
iea <- iea %>% 
  mutate(indirect_CO2=ifelse(flow=="TOTIND",NA,indirect_CO2)) %>% 
  mutate(indirect_CO2=ifelse(flow=="TOTTRANS",NA,indirect_CO2)) %>% 
  mutate(indirect_CO2=ifelse(flow=="MANUFACT",NA,indirect_CO2)) 

blarg <- iea %>% 
  filter(country=="World") %>% 
  filter(year==2019)

###### allocate other non specified (ONONSPEC) indirect CO2 to sectors
...
# aggregate data to subsectors

indirect_CO2_countries <- indirect_CO2_countries %>% 
  group_by(across(c(-iea_code,-sector_code,-description,-CO2_indirect))) %>% 
  summarise(CO2_indirect=sum(CO2_indirect)) %>%
  ungroup() %>% 
  select(-chapter,-subsector) %>% 
  rename(sector_title=chapter_title)
...
save(indirect_CO2_regions,indirect_CO2_world,indirect_CO2_countries,file="../../Data/data_indirect_co2.RData")

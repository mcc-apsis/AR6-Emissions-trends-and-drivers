...
crf_methane_adjustment<-gather(crf_methane_adjustment,key = "year", value = "value", "1986":"2019") %>%
  ...
  mutate(unit="Gt") %>%
  mutate(year=as.numeric(year)) %>%
  mutate(gwp_crf_ar6=value*(29.8-27.2)) %>%         #in the first step all methane emissions were factored by 27.2; for those sectors, where the higher factor of 29.8 should be used, this is added
...
combined_crf_edgar <- left_join(crf_total_sector,edgar_data,by=c("ISO","year","gas")) %>%
  rename("EDGAR"="value","CRF"="gwp_crf") %>%
  filter(year>=1990) %>%
...
plot_diff<-function(GAS){
  ...
  p<-ggplot(dat, aes(x=country,y=value,fill=data)) + geom_bar(stat = "identity", position=position_dodge()) +
    ...
    ggtitle(paste("2019", GAS, "data in GtCO2eq/yr", sep=" ")) +
    plot_theme
  return(p)
}
...
box_diff<-function(GAS){
  ...
  p<-ggplot(dat, aes(x=country,goup=country,y=diff_p)) + geom_boxplot() +
    ...
    ggtitle(paste("Annual differences in", GAS, " data between 1990 and 2019 (in %)", sep=" ")) +
    plot_theme
  return(p)
}
...
```{r gas_trends_spm, echo=FALSE,warning=FALSE,fig.width=10,fig.height=14,fig.path="../Analysis and figures for ESSD paper/Results/Plots",dev=c('png','pdf')}

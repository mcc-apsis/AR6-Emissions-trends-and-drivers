...
ipcc_regions <- ipcc_regions %>% select(-region_ar6_6_short,-region_ar6_10_short)

gwps_ch4 <- gwps_ch4 %>% select(-value,-gwp_ar6,-gwp_ar4,-gwp_ar2)
gwps <- gwps %>% select(gas,gwp_ar5)
...
edgar_raw <- edgar_raw %>% 
  select(-region_ar6_6_short,-region_ar6_10_short,-chapter,-subsector,-gwp100_ar2,-gwp100_ar4,-gwp100_ar6)

edgar_raw <- edgar_raw %>% 
  group_by(ISO,country,region_ar6_6,region_ar6_10,region_ar6_22,region_ar6_dev,year,sector_title=chapter_title,subsector_title,gas,gwp100_ar5) %>% 
...
info_raw = data.frame("x" = c("Data description","Global warming potentials","Sectors","Regions","Author & contact","R code","Land use data","","Last date of compilation","","","Sources","Link to public EDGAR version"),
                  "y" = c("This data file provides global, regional, national, and sectoral estimates of greenhouse gas (GHG) emissions from 1970-2019",
                          "Emissions by gas are provided in tons of original units. For convenience, we also include 100 year global warming potentials from the IPCC 5th Assessment Report as a column in the dataset. Please see the separate file 'essd_ghg_data_gwp100.xlsx' for data already aggregated to 100 year global warming potentials.",
...
meta_raw <- data.frame("Variable" = c("gas","gwp100_ar5","value"),
                       "Description" = c("Greenhouse gas (CO2, CH4, N2O or Fgas)","Global warming potential with a 100 year time horizon, AR5 values","Total value for each sector"),
...
writeData(wb1, sheet = "data", edgar_raw, colNames = T)
...
edgar_ghg <- edgar_ghg %>% 
  group_by(ISO,country,region_ar6_6,region_ar6_10,region_ar6_22,region_ar6_dev,year,sector_title=chapter_title,subsector_title) %>% 
...
info_ghg = data.frame("x" = c("Data description","Global warming potentials","Sectors","Regions","Author & contact","R code","Land use data","","Last date of compilation","","","Sources","Link to public EDGAR version"),
                      "y" = c("This data file provides global, regional, national, and sectoral estimates of greenhouse gas (GHG) emissions from 1970-2019",
                              "Emissions by gas are provided in tons of CO2 equivalent, based on 100 year global warming potentials from the IPCC 5th Assessment Report. We also aggregate all Fgases into a single variable. Please see the separate file 'essd_ghg_data.xlsx' for data in original gas units.",
                              "Emissions have been allocated to major sectors corresponding to the IPCC AR6 chapters on consultation with the chapter leads. A further breakdown by subsector is also available. A full list and description of how emissions codes match to sectors and subsectors is in the sector_classification tab.",
                              "Countries have been allocated to regions by the IPCC AR6 WGIII Technical Support Unit. A summary of the country and region codes is in the region_classification tab.",
                              "William F. Lamb (Lamb@mcc-berlin.net) compiled this data from underlying sources (see metadata).",
                              "The code for compiling this data (as well as summary figures for AR6 Ch2) is available online: https://github.com/mcc-apsis/AR6-Emissions-trends-and-drivers",
...
land <- land %>% select(-region_ar6_6_short,-region_ar6_10_short)

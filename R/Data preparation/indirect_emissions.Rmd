...
indirect_CO2_countries <- indirect_CO2_countries %>% 
  select(country,ISO=alpha.3,region_ar6_5,region_ar6_10,region_ar6_22,region_ar6_dev,region_ar6_5_short,year,everything()) %>% 
  select(-name)
...
info = data.frame(x=c("Author","Date","Code"),y=c("William F. Lamb",as.character(Sys.Date()),"https://github.com/mcc-apsis/AR6-Emissions-trends-and-drivers/blob/master/R/Data%20preparation/indirect_emissions.Rmd"))
...
openxlsx::saveWorkbook(wb,paste0("../../Results/Data/ipcc_data_indirect_emissions.xlsx"),overwrite=T)

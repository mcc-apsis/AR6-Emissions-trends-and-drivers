...
waterfall_data <- edgar_raw %>% 
  ...
  group_by(year,gas,gwp100_ar2,gwp100_ar4,gwp100_ar5,gwp100_ar6) %>% 
...
waterfall_data<-waterfall_data %>%
  mutate(gwp=ifelse(gwp=="gwp_ar2","GWP AR2",gwp)) %>%
  mutate(gwp=ifelse(gwp=="gwp_ar4","GWP AR4",gwp)) %>%
  mutate(gwp=ifelse(gwp=="gwp_ar5","GWP AR5",gwp)) %>%
  mutate(gwp=ifelse(gwp=="gwp_ar6","GWP AR6",gwp))
...
p_waterfall <- waterfall_data %>%
  ...
  #geom_text(data=waterfall_data %>% filter(gwp!="gwp_ar4") %>%  filter(gas=="CH4"),aes(x=gas,xlab(c("AR2 GWP","AR5 GWP","AR6 GWP"))),size=3.5,colour="#252525") +
   ...
   theme_bw() +
   scale_y_continuous(breaks=c(0,10,20,30,40,50,60),limits=c(0,66.2)) +
   ...
   theme(
     ...
     #axis.text.x = element_text(c("AR6 GWP","AR5 GWP","AR4 GWP","AR2 GWP")))
...
growth_data <- plot_data %>%
  ...
  mutate(decade=ifelse((year>2008 & year<2019),"2009-2018",decade)) %>%
  group_by(decade) %>%
...
p_gas_totals <- plot_data %>% 
  ...
  geom_text(data=shares,aes(x=ifelse(year<2018,year+1.5,year+2.5),y=location,label=paste(round(fractions,0),"%",sep="")),size=3.5,colour="#252525")+
  
  #### text with the totals
  geom_text(data=shares %>% filter(gas=="Fgas") %>% filter(year<=2018),aes(x=year,y=62,label=paste(round(totals,0),  "Gt")),size=3.5,colour="#252525")+
...
countries<-left_join(countries,crf_ghg,by=c("ISO","year"))%>%
...
countries <- countries %>% 
  ...
  #mutate(avg_ghg_growth=(last(GHG)/first(GHG))^(1/(last(year)-time_start))-1) %>% 
  mutate(abs_ghg_growth=last(GHG)-first(GHG)) %>%
  mutate(avg_ghg_growth_crf=ifelse(is.na(GHG_crf),NA,growth_rate(years = year,y=GHG_crf))) %>%
  #mutate(avg_ghg_growth_crf=(last(GHG_crf)/first(GHG_crf))^(1/(last(year)-time_start))-1) %>% 
  mutate(abs_ghg_growth_crf=last(GHG_crf)-first(GHG_crf)) %>%
  filter(year==2018) %>% 
  mutate(avg_growth=avg_growth*100)%>% 
...
p_countries_rate <- countries %>%
  ...
  annotate(geom = 'text', label = 'Years: 2009-2018',
...
openxlsx::addWorksheet(wb,"sectors_2018")
openxlsx::writeData(wb, sheet = "sectors_2018", sectors, colNames = T, rowNames = F)

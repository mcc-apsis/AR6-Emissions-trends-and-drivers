...
growth_data <- sector_data %>%
  ...
  mutate(decade=ifelse((year>=2009 & year<=2018),"2009-2018",decade)) %>%
  group_by(decade) %>%
...
p1 <- sector_data %>%
  ...
  geom_text(data=shares %>% filter(chapter_title==shares$chapter_title[1]),aes(x=year,y=1.2*max(shares$totals),label=paste(round(totals,0),  "Gt")),size=3.5,colour="#252525") +
  ...
  scale_x_continuous(breaks=c(1970,1980,1990,2000,2010,2018),limits = c(1970,2020)) +
  scale_y_continuous() +
...
subsector_data <- subsector_data %>% 
  ...
  mutate(avg_growth_1990=growth_rate(years = year,y=GHG))%>%
  #mutate(avg_growth_1990=(last(GHG)/first(GHG))^(1/(last(year)-time_start))-1) %>% 
  mutate(abs_growth_1990=last(GHG)-first(GHG)) %>%
  filter(year>=2009) %>% 
  group_by(subsector,subsector_title) %>% 
  mutate(avg_growth_2009=growth_rate(years = year,y=GHG))%>%
  #mutate(avg_growth_2009=(last(GHG)/first(GHG))^(1/(last(year)-2009))-1) %>% 
  mutate(abs_growth_2009=last(GHG)-first(GHG)) %>%
  filter(year==2018) %>% 
...
subsector_data <- gather(subsector_data,var,value,avg_growth_1990,abs_growth_1990,avg_growth_2009,abs_growth_2009)

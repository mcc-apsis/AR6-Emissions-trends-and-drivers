...
edgar_by_gas<-edgar_by_gas %>%
  ...
  mutate(per_gdp_real=value/gdp_real) %>%
  mutate(per_gdp_ppp=value/gdp_ppp)
...
# colours = c("#a6d854","#e78ac3","#8da0cb","#fc8d62","black")
...
plot_theme <- theme_bw() +
 ...
 p<-edgar_by_gas %>%
    #filter(gas==gas1) %>%
    ggplot(.,aes(x=year,y=absolute,color=gas))+
    #geom_line(color=col)+

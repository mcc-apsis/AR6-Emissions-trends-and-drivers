...
```{r plot_ipcc_sm,echo=FALSE,warning=FALSE,include=FALSE}
...
```{r Chap2_SM_CO2eq,echo=FALSE,warning=FALSE,fig.width=8,fig.height=12,fig.path="Results/Plots/",dev=c('png','pdf')}

p_abs+p_pc+p_pgdp

```

```{r gas_data_raw,echo=FALSE,warning=FALSE,include=FALSE}

edgar_by_gas<-edgar_raw %>%
...
edgar_by_gas<-edgar_by_gas %>%
  mutate(per_capita=absolute/population*1e3) %>%
  mutate(per_gdp_real=(absolute/gdp_real)*1e6) %>%
  #mutate(per_gdp_ppp=(absolute/gdp_ppp)*1e3)
  mutate(per_capita_high = absolute_high/population*1e3) %>% 
  mutate(per_capita_low = absolute_low/population*1e3) %>% 
  mutate(per_gdp_high = (absolute_high/gdp_real)*1e6) %>% 
  mutate(per_gdp_low = (absolute_low/gdp_real)*1e6) %>% 
 # rename("absolute"="value") %>%
  mutate(absolute=absolute/1e6)%>%
  mutate(absolute_high = absolute_high/1e6) %>% 
  mutate(absolute_low = absolute_low/1e6) %>% 
  select(-uncertainty)

```

```{r plot_ipcc_sm2,echo=FALSE,warning=FALSE,include=FALSE}
...
p_abs<-edgar_by_gas %>%
  ...
  # ),fill = "white",alpha=0.04) +
    ...
    #ylab('GtCO2eq') +
    facet_grid(gas~.,scales = "free",switch = "y") +
    ggtitle("Absolute emissions in Mt") +
    plot_theme
...
p_pc<-edgar_by_gas %>%
  ...
  # ),fill = "white",alpha=0.04) +
    ...
    #ylab('tCO2eq per capita') +
    facet_grid(gas~.,scales = "free") +
    ggtitle("Emissions per capita in kg/cap") +
    plot_theme+
    theme(strip.text.y = element_blank())
...
p_pgdp<-edgar_by_gas %>%
  ...
  # ),fill = "white",alpha=0.04) +
    ...
    #ylab('tCO2/GDP') +
    facet_grid(gas~.,scales = "free") +
    ggtitle("Emission intensity in g/$") +
    plot_theme+
    theme(strip.text.y = element_blank())
...
```{r Chap2_SM_nativeunits,echo=FALSE,warning=FALSE,fig.width=8,fig.height=4,fig.path="Results/Plots/",dev=c('png','pdf')}

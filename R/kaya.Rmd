...
kaya_data <- openxlsx::read.xlsx("../Data/kaya_data_flat.xlsx")
...
# join IPCC regions
kaya_data <- left_join(kaya_data,tsu_codes %>% select(ISO,name,region_ar6_5,region_ar6_5_short,region_ar6_10),by=c("ISO"="ISO"))
...
kaya_data <- kaya_data %>% 
  ...
  mutate(region_ar6_5_short=ifelse(ISO=="WLD","World",region_ar6_5_short)) %>% 
...
# openxlsx::writeData(wb2, sheet = "data",kaya %>% filter(sector!="AFOLU"), colNames = T, rowNames = F)
...
kaya_plots_10 <- function(kaya,sector_label) {
  ...
  kaya_plot_data <- gather(kaya_plot_data %>% select(-GDP,-energy),var,value,GDP_POP,POP,CO2,CO2_energy,energy_GDP)
  ...
  p <- kaya_plot_data %>%
    ...
    geom_text(data=kaya_plot_data %>% filter(year==2017),
...
openxlsx::saveWorkbook(wb,paste0("../Results/Kaya/sector_kaya_analysis_",Sys.Date(),".xlsx"),overwrite=T)
...
openxlsx::saveWorkbook(wb,"../Results/Kaya/kaya_tests.xlsx",overwrite=T)
